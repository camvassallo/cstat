...
Run:  cd training && ROSTER_VALUE_FEATURES=1 .venv/bin/python experiments/experiment_game_value_features.py
...
# Everything below runs only as a script. Module-level before #364, which made
# `import experiment_game_value_features` start a two-hour matrix build; the
# experiments/ import check needs the module to load without doing that.
def main():
