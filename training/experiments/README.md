...
| `experiment_od_anchor.py` | Layer 4 on the split: should the AdjO half be anchored as hard as the net (refit / scaled / overhaul-only / new-HC-unanchored weights), and should AdjD be derived, direct, or both halves direct and reconciled to the net? WF through the served blend on the served ex-ante frame. | 2026-09-22 | **Weight changes on the O half alone REJECTED** — they move error between O and derived D (`err_D = err_O − err_net`): scale 0.75 gives AdjO −0.023 (z −2.7) and derived AdjD +0.118 on top-25 offences (z +3.1); the per-fold refit is a tie. **Reconciled NET + O + D is the finding**: a direct relative-AdjD model with its own anchor, both halves nudged by half the net residual so O' − D' ≡ the served net. AdjD 3.666 → 3.553 (−0.113, z −6.2, 6/6 seasons), AdjO 3.931 → 3.865 (−0.066, z −3.6), prior top-25 defence AdjD −0.222 / AdjO −0.191, new HC AdjD −0.134, net untouched, identity exact. Direct D alone is −0.076 but leaves a 1.28-point O − D − net gap on the page. Candidate to ship: a third calibrator on the existing 27-slot contract plus a serve-time reconciliation. | `eval_history/od_anchor_20260922_summary.json` |
| `experiment_od_inputs.py` | Do roster CAM-O / CAM-D aggregates (prior-season halves, and the projected net split by each player's prior share) help the AdjO half, a direct AdjD, or the net? Same architecture, more information; WF through the served blend, ex-post SQL frame as a paired screen. | 2026-09-22 | **REJECTED — marginal.** AdjO −0.035 MAE pooled (z=−2.2, 5/6 seasons), derived AdjD −0.024 (z=−1.5); net unmoved; direct AdjD worse with the split features (+0.033, z=+2.1). Nothing on the cohort that motivated it (prior top-25 defence programs: +0.02, z=+0.3) — the O-vs-D allocation there is the anchors', not the features'. A ~1% gain does not buy a 27→37 serving-contract change; the Saint Mary's-shaped lever is the AdjO anchor's turnover ramp, not the inputs. | `eval_history/od_inputs_20260922_summary.json` |
