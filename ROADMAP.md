...
- [~] **Freshman recruiting ingest + tier-mean heuristic + per-player prior model**. The /projections page now consumes a third roster source. **Ingest shipped: 1,200 recruits across class-of-2024/2025/2026 (321 + 512 + 367). Tier-mean heuristic shipped: 558 of those recruits paired to actual freshman cstat-seasons (224 from 2024 → cstat-season 2025 freshmen; 334 from 2025 → cstat-season 2026 freshmen; class-of-2026 hasn't played yet) → 4-tier population profile keyed on composite_rank. Per-player prior model deferred.**
  ...
  **Remaining work** (separate PRs, in rough priority order):
  ...
  - [ ] **Leave-one-class-out CV for the freshman model** (true out-of-sample validation). Today's 5-fold random CV gives MAE 2.49, in-sample on 2025 gives MAE 2.01 — that's the gap we can measure, but the 2025 class is in the training set so it's not a real held-out test. The honest validation: retrain 4 times, each excluding one class (2022/2023/2024/2025), measure MAE on the held-out cohort. Expected outcome: higher than 5-fold CV (because adjacent-class signal leaks out), but should still beat the tier-mean baseline of 2.56 on the held-out cohort. Wire as a `--leave-out YYYY` flag in `training/train_freshman_model.py`; record per-fold MAE in the meta JSON next to `cv_5fold`. If LOCO MAE blows past 3.0 we have a serious overfit signal; if it lands ~2.5–2.7 the model generalizes.
