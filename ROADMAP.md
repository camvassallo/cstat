...
- [ ] **Archetype-aware team views & Team Compare** (player-filter enhancements + 12-axis roster viz + 2-team compare + enhanced Predict):
  - **Players-tab filter polish**: extend today's `?archetype=` filter (primary, or primary-OR-secondary via the toggle — `ROADMAP.md:383`) with a **primary+secondary combination** mode for hybrid drill-downs (e.g., "Wizard/Bard"). UI: second class-chip slot in the page header with a mode selector (`primary only` / `primary or secondary` / `primary + secondary`).
