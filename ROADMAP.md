...
- **Trajectory projections for historical transfer/projection classes are in-sample.** `trajectory_model_meta.json::seasons_trained_on = [2022, 2023, 2024, 2025, 2026]` means the deployed model was fit on all four pairs `2022→2023` / `2023→2024` / `2024→2025` / `2025→2026` (total n=9,239) — the exact transitions the Transfer page and 2027 projection page display. Features are honest (prior-season only, no future-data leak), but the model has memorized each historical player's actual next-season CamPom outcome. Honest LOPO MAE is 2.20 pooled; per-bucket in-sample MAE is ~1.8 (~20% gap). Gap widens at the elite tail (+15..+20 has only n=20 training rows; +20+ has n=1). Visible symptom: elite 2024 transfers like Richie Saunders, Joshua Jefferson, and Oscar Cluff project far above their 247 rankings because the model already saw their breakout 2025 outcomes. Only the 2026 portal class (projection target 2027, not yet played) is genuinely out-of-sample. Same caveat applies to the freshman-impact model serving class-of-2021..2025 projections; class-of-2026 (target 2027) is the only OOS recruit cohort. → see "Serve held-out trajectory/freshman predictions for historical years" in Refactor Backlog.
...
The trajectory and freshman-impact models were trained on every pair of seasons currently in the DB (`trajectory_model_meta.json::seasons_trained_on = [2022, 2023, 2024, 2025, 2026]` → pairs `2022→2023`, `2023→2024`, `2024→2025`, `2025→2026`; freshman model trains on class-of-2021 through class-of-2025). The Transfer page (`/api/transfers/{year}`), 2027 projection page (`/api/projections/{year}/teams/{team_id}`), Recruits page (`/api/recruits/{year}`), and PlayerDetail/PlayerProgression "Proj YYYY-YY" chip all call the full model — for any historical year (2022-2025 transfers, class-of-2021..2025 recruits), that's in-sample inference on rows the model already learned from. Honest gap is ~20% on trajectory (in-sample ~1.8 MAE vs LOPO 2.20 pooled) and widens at the elite tail where there are only n=20 training rows in `+15..+20` (and n=1 in `+20+`). Symptom: elite 2024 transfers like Richie Saunders, Joshua Jefferson, Oscar Cluff project far above their 247 rankings because the model saw their actual 2025 outcomes.
