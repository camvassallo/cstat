...
- [~] **Freshman recruiting ingest + tier-mean heuristic + per-player prior model**. The /projections page now consumes a third roster source. **Ingest shipped: 1,200 recruits across class-of-2024/2025/2026 (321 + 512 + 367). Tier-mean heuristic shipped: 558 of those recruits paired to actual freshman cstat-seasons (224 from 2024 → cstat-season 2025 freshmen; 334 from 2025 → cstat-season 2026 freshmen; class-of-2026 hasn't played yet) → 4-tier population profile keyed on composite_rank. Per-player prior model deferred.**
  ...
  **Remaining work** (separate PRs, in rough priority order):
  ...
  - [ ] **← UP NEXT: Projection-page swap — replace tier-mean in `synthesize_freshman_row`**. The freshman model predicts CamPom; `synthesize_freshman_row` needs per-game stats (ppg/rpg/ts%/usg/etc.) to feed `build_roster_features`. **Cheap path** (recommended v1): linearly scale the tier's per-game profile so its derived CamPom matches the predicted mean — preserves the tier-mean's per-stat *shape* but anchors the magnitude to the per-recruit prediction. **Principled path** (deferred): train multi-output LightGBMs per stat. **Plumbing**: `synthesize_freshman_row(recruit_id, tier)` becomes `synthesize_freshman_row(predictor, recruit)` where `recruit` carries the raw fields; the function calls `predictor.predict_freshman_batch` (single-row but uses the same code path) and scales the tier profile. **Regression test**: snapshot the current `/api/projections/2027` AdjEM numbers per team, swap, recompute, diff. Expect material movement on teams with elite incoming classes (Duke, Kansas, Arkansas in 2026) and minimal change on teams with mid-tier classes. **Honesty caveat**: the scaling heuristic assumes per-stat ratios stay constant within tier — true on average but wrong for specialized profiles (e.g., a 7-footer who projects to high blocks would still get the tier-mean BLK%). Multi-output is the fix; the cheap path is "good enough until it isn't."
