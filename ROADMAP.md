...
- **Freshman-impact prior model: 1.1% pooled lift out-of-sample (vs 2.9% in-sample), 9.6% on T1 LOCO.** Wins big on the top-30 cohort (the headline bucket) and T4 unranked; T2/T3 essentially flat in LOCO. The in-sample 2.9% headline lift overstated real-world performance — leave-one-class-out CV (every class held out, retrained on the rest, scored on held-out) gives pooled MAE 2.534 vs tier-mean baseline 2.561. Model wins 3 of 4 holdouts; 2023 ties baseline (cohort-specific patterns that don't generalize cleanly from the others). Gap to 5-fold random CV is only +0.046 — no severe overfit, just mild fold-overlap leakage. Limited by the 963-row training corpus from 4 paired classes; historical recruit backfill (class-of-2021 and earlier) is the obvious next unlock. Selection bias on top recruits who returned for sophomore year is sharper here than for trajectory: elite freshmen leave, so the model is calibrated on a thinner cohort than headline MAE implies. → see "Freshman-impact prior model" in §6.
- **Trajectory projections for historical transfer/projection classes are in-sample.** `trajectory_model_meta.json::seasons_trained_on = [2024, 2025, 2026]` means the deployed model was fit on pairs `2024→2025` (n=2311) and `2025→2026` (n=2113) — the exact transitions the Transfer page and 2027 projection page display. Features are honest (prior-season only, no future-data leak), but the model has memorized each historical player's actual next-season CamPom outcome. Headline in-sample MAE ~1.5 per class-year bucket; honest LOPO MAE is 2.27 pooled (~35% gap). Gap widens at the elite tail (+15..+20 has only n=13 training rows). Visible symptom: elite 2024 transfers like Richie Saunders, Joshua Jefferson, and Oscar Cluff project far above their 247 rankings because the model already saw their breakout 2025 outcomes. Only the 2026 portal class (projection target 2027, not yet played) is genuinely out-of-sample. Same caveat applies to the freshman-impact model serving class-of-2024/2025 projections. → see "Serve held-out trajectory/freshman predictions for historical years" in Refactor Backlog.
