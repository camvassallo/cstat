...
- [ ] **CamPom overfitting audit & point-in-time predict backtest** — the §4f experiment compared CamPom-vs-raw-GBPM as predict features but didn't isolate *temporal* leakage. `torvik_player_stats.cam_gbpm_v3` is a full-season aggregate, so when the LightGBM trains on a December 2023 game using those values, it's seeing March 2024 player ratings to predict December. Both training and the current temporal-split eval (held-out AUC 0.764) are likely inflated by this. Three nested tests, ranked by ROI:
  - **Constants sensitivity sweep** *(cheap, ~half a day)*: perturb each `CAMPOM_*` const in `compute.rs:1118-1132` (`OFFENSE_EXPONENT`, `USG_REF`, `MINUTES_EXPONENT`, `GP_K`, `SOS_TRANSFER_RATE`, `PLAYER_SOS_TRANSFER_RATE`) by ±20% one at a time, recompute, retrain the predict LightGBM, log AUC delta. A robust composite should be near-flat (AUC swing <0.005); per-knob swings ≥0.01 signal the constants were fit to outcomes in-sample. Output: a small CSV/markdown sensitivity table to drop into `docs/campom_methodology.md`.
  - **Leave-one-season-out (LOSO) game-prediction backtest** *(medium, ~1–2 days)*: the trajectory/freshman/roster-impact models already have LOSO scaffolding (`training/train_*_model.py`); port the same pattern to `train.py`. Train on N-1 seasons, evaluate cold on N, repeat. Different from the current chronological split because no in-season info leaks. Expectation: AUC drops to ~0.74 (would still beat NatStat ELO 0.722); if it craters below 0.70, the current eval is more inflated than the leakage analysis above suggested. Persist per-season OOF predictions into a new `game_oof_predictions` table mirroring `trajectory_oof_predictions` so the model-accuracy-dashboard item above has honest historical numbers to consume.
  - **Point-in-time CamPom backtest** *(big, ~3–5 days, gold standard)*: the only test that fully isolates lookahead. Steps: (a) new `torvik_player_game_stats` table — wide, ~50 numeric cols matching `TorkvikGameRow`; (b) extend `cstat-ingest torvik` to persist game rows (the data is already fetched, currently discarded after rebound backfill — see `ingest/torvik.rs:223`); (c) `compute_campom_at(as_of_date)` variant in `compute.rs` that aggregates only games before the cutoff; (d) regenerate `features.py` training matrix using cumulative-only CamPom per game; (e) retrain and compare. AUC delta vs. full-season CamPom is the answer. Storage: ~155k player-games/season × 12 seasons × ~600 bytes/row indexed = **~1.1 GB**; not prohibitive. The standalone "Native cstat player impact metric" item below becomes much easier to validate against a point-in-time benchmark too.
  - **Data acquisition — what we considered and why**:
    - *Date-spliced Torvik JSON URLs*: not viable. `docs/torvik-api-guide.md` line 27 is explicit — "any date-range filtering must be done client-side after ingestion." The HTML pages take `start=/end=` params but scraping ~150 dates × 12 seasons × multiple endpoints is brittle and the contract isn't stable. Skip.
    - *Per-game Torvik data we already pull*: the right answer (above). Zero new API calls — `{year}_all_advgames.json.gz` is one request per season we already make. The full per-game row carries OGBPM/DGBPM/usage/min — everything CamPom needs.
    - *NatStat box scores* (`player_game_stats`): available today but crude — box-score-derived linear coefficients are exactly what the prior native-BPM attempt failed at (r=0.075 with Torvik OBPM, per §6 "Native cstat player impact metric"). Don't repeat. Reserve for post-PBP stint-level attribution.
    - *NatStat PBP*: a different unlock (stint-level on/off attribution). Already queued separately as "Play-by-play ingestion" in Refactor Backlog + "PBP-derived features" in Phase 6. Don't conflate with the temporal-leakage fix.
  - **Acceptance**: ship at least the constants sweep + LOSO before claiming the predict model meaningfully beats NatStat ELO (current side-by-side at AUC 0.764 vs 0.722 is suggestive but not honest until LOSO confirms). Point-in-time CamPom is the polish step and unblocks the calibration dashboard above.
- [ ] **ATS backtest harness** — reusable measurement infrastructure for "would cstat beat Vegas?" Sibling to the overfitting audit above: that one measures model calibration, this one measures betting edge against the market. They share the point-in-time CamPom infra and are the joint gating signal for the calibration dashboard. **Smoke test on 400 random games returned 74.4% ATS / +42% ROI, with the |edge|≥8 bucket at 95.1% — almost entirely lookahead** (`/api/predict` reads end-of-season team state for historical games). The harness exists to produce the *honest* version of that number, not to chase the inflated one. Frame as research/measurement infrastructure, not a betting product.
