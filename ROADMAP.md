...
- [x] **Sortable-table follow-ups** (small polish items uncovered during the table polish work):
  ...
  - [x] **`pctileTextColor` input clamp**: defensive `Math.max(0, Math.min(1, p))` at the function entry in `TeamDetail.tsx`.
...
- [x] API: `GET /api/players/:id/archetype`, `GET /api/players/:id/similar?k=10`, `GET /api/archetypes` (class glossary + exemplars)
