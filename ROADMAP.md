...
- [x] **Migrate `/api/transfers/{year}` route from embedded JSON to DB**. `crates/cstat-api/src/routes/transfers.rs` now reads from the `transfers` table via `sqlx::query_as` (no more `include_str!` / `EMBEDDED_TRANSFERS`). Response shape is unchanged except `rank_247` is now nullable — the embedded JSON era was a top-N scrape with rank always present, the DB carries the full portal (1,497 rows for 2026: 808 ranked + 689 unranked), so the long tail now appears. Frontend type + Δ-column null check + 247-rank cell updated. Column choice: `transfer_rank` (247's within-portal rank), not the composite `rank` — gives bit-for-bit parity with the old top-N for the rows that overlap (0/247 names missing from new ranked set; 93 extra ranked rows + 1,157 unranked rows added). 2024 and 2025 currently 404 until raw snapshots are captured and re-ingested via the same `--bootstrap-from` path; the prior per-year files in `data/transfers/{2024,2025}.json` were a different shape (top-N scrape, not the 247 API envelope) so they can't seed the new pipeline. Prod swap pending: needs `cstat-ingest transfers --year 2026 --bootstrap-from data/transfers/2026_raw.json` against `PROD_DATABASE_URL` *before* deploying the binary, or the 2026 dropdown will 404 between deploy and seed. Same DB-backing pattern applies cleanly to the draft tables (`draft_early_entrants`, `draft_big_board`) once those have a regular refresh cadence — out of scope for this batch.
