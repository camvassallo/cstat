...
- **Trajectory projections for historical transfer/projection classes are in-sample (foundation shipped, activation pending retrain).** Until the next training run populates `trajectory_oof_predictions` / `freshman_oof_predictions`, every historical-year route still serves in-sample inference. The schema, persistence, and OOF-first route precedence flips are all in (see "Serve held-out trajectory/freshman predictions" in Refactor Backlog) — the next `python training/train_trajectory_model.py` + `train_freshman_model.py` run activates them. Symptom for now: elite 2024 transfers like Saunders / Jefferson / Cluff still project +15-20 instead of the LOPO-honest +8-12. Forward year (2026→2027) was always out-of-sample and stays unchanged through the transition.
...
1. Run `python training/train_trajectory_model.py` and `python training/train_freshman_model.py` against a `DATABASE_URL` so the tables get populated. Trajectory retrain is the heavier one — 4 LOPO mean + 4 LOPO q10 + 4 LOPO q90 fits on ~5k rows each (~15-20 min total). Freshman LOCO is ~5-10 min.
2. Add a boot-validator gate: extend `validate_trajectory_meta` / `validate_freshman_meta` in `inference.rs` to require `meta["oof_persisted"] == true`. Safe to add only AFTER step 1 commits new meta JSONs to git — adding the validator first would fail the API boot against the currently committed metas (which lack the key).
3. Validation: `curl localhost:8080/api/transfers/2024 | jq` and confirm Saunders / Jefferson / Cluff `projected_campom_mean` drops from inflated (~+15-20 in-sample) to LOPO-honest (~+8-12). Spot-check `/api/projections/2026/teams/{duke_id}` still shows live-inference values for class-of-2026 returners projecting into 2027 (forward year, OOF table empty for target_season=2027).
