...
- [~] **Freshman recruiting ingest + tier-mean heuristic + per-player prior model**. The /projections page now consumes a third roster source. **Ingest shipped (class-of-2024/2025/2026). Tier-mean heuristic shipped (1,200 recruits joined to actual freshman seasons → 4-tier population profile keyed on composite_rank). Per-player prior model deferred.**
  ...
  **Remaining work** (separate PRs, in rough priority order):
  - **Freshman-impact prior model** — the year-0 → year-1 per-player projection (upgrade from the tier-mean heuristic above). Training pairs: (composite_rank, position, height/weight, committed-school AdjEM tier) → actual freshman-season CamPom v3. We now have 558 paired rows from class-of-2024 + class-of-2025; on the thin side for tight quantile bands, but enough for a usable point estimate + wide bands. Output: predicted freshman CamPom v3 + quantile-regression bands. Honesty framing: rank → impact correlation is strong at the top (Five-Star → All-American is reliable) and noise below ~30th-ranked; bands must widen accordingly. Train/serve contract: `recruit_prior_meta.json` with Rust inference path drift-check at boot. Once shipped, swap `synthesize_freshman_row` from tier-mean to per-recruit prediction.
