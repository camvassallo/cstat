...
- [ ] **Predict page revamp** (UI redesign + backend + retrained model). Today's Predict is a thin wrapper over `GET /api/predict` that returns a margin and win%. Targeting a richer matchup view that's the de-facto destination for "what does the model say?", with the prediction sitting *inside* the comparison context rather than above it:
  ...
  - **Explainability** (subsumes the previously-standalone "Game prediction explainability" bullet): export SHAP values from training, expose via API, render a horizontal bar breakdown of the top contributing features ("Duke +5 from GBPM gap, +3 from defense, −2 road game") beneath the headline. SHAP attribution + Team Compare panel are complementary — one explains "what the model saw," the other explains "what's actually different about the rosters."
