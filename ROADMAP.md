...
- [~] **Freshman recruiting ingest + tier-mean heuristic + per-player prior model**. The /projections page now consumes a third roster source. **Ingest shipped: 1,200 recruits across class-of-2024/2025/2026 (321 + 512 + 367). Tier-mean heuristic shipped: 558 of those recruits paired to actual freshman cstat-seasons (224 from 2024 → cstat-season 2025 freshmen; 334 from 2025 → cstat-season 2026 freshmen; class-of-2026 hasn't played yet) → 4-tier population profile keyed on composite_rank. Per-player prior model deferred.**
  ...
  **Remaining work** (separate PRs, in rough priority order):
  ...
  - [ ] **← UP NEXT: Per-recruit CamPom surface on the Recruits tab**. Now unblocked by the Rust inference module above. Plumb `freshman_model::fetch_freshman_features` + `Predictor::predict_freshman` through the existing recruits route (`crates/cstat-api/src/routes/recruits.rs`); extend `GET /api/recruits/{year}` response with `projected_campom_mean / lower / upper`. Frontend (`web/src/pages/Players.tsx` Recruits tab) renders a CamPom chip per recruit row alongside the 247 rank — same value-index narrative as TransferPortal's Δ-vs-247-rank column. Smaller scope than the projection-page swap below; ships real per-recruit projections to users without touching the team-AdjEM aggregation. Honest framing in the chip tooltip: mean (q10–q90) band, note the selection-bias caveat for elite recruits (calibrated on returners).
