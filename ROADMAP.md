...
- [ ] **Projected impact at destination (Δteam rating)**: replace the raw 247 vs CamPom delta with a destination-aware projection. Two framings, sharing one projection engine.
  ...
  - [~] **Projection bias fix — top teams systematically under-projected** *(in progress, 2026-05-20)*. Spot-check of `/api/projections/2027`: **all 25** of 2026's top teams by AdjEM project *worse* for 2027; top-quartile mean Δ −7.66 vs bottom-quartile +0.65, league-wide −3.29. Root cause is **not** error in `predict_adj_em` — it's that `compose_all_projections` hands the model a roster that isn't a real roster. `ProjectedRoster::for_scenario` materializes every player at their *prior* minutes: returners at last year's role, recruits at a tier-fixed MPG, arrivals at their *source-team* MPG. Nobody is promoted into the minutes vacated by departed seniors / drafted players, so a gutted roster sums to ~150 player-minutes and a portal-stacked one past 230 — both well outside the ~221 Σmpg / ~6,100 `total_minutes` the roster model trained on (calibrated from 1,090 team-seasons, 2024–26). Purdue — a +36 AdjEM team — scored at **raw +3** purely because its projected roster summed to ~150 MPG. The freshman + trajectory models both emit `cam_v3`, which the box-score roster model never reads, so their per-player resolution is discarded. Fix lands in two phases:
    ...
    - [~] **Phase B — impact-aggregation projection model** *(in progress, scoped 2026-05-20)*. The box-score roster model is the wrong consumer for the `cam_v3`-emitting trajectory/freshman models. Train a dedicated bottom-up model: features = aggregated *projected* `cam_v3` of the roster (Σ, minutes-weighted mean, top-1/3/7, counts over thresholds) + minutes concentration + archetype balance + experience mix; target = `team_season_stats.adj_efficiency_margin`. At serve time feed projected `cam_v3` (trajectory for returners/arrivals, freshman for recruits — already computed per-player on the detail route). Keep the box-score roster model for the swap-Δ tool. This is a model-of-a-model by design — legitimate stacking, not a smell, as long as the composed pipeline is backtested end-to-end against actual 2025 and 2026 AdjEM (acceptance criterion).
      - **Why this isn't the "identity-collapse" trap the box-score model avoids**: `train_roster_model.py` deliberately excludes `cam_v3`/GBPM because `Σ(cam_v3 × minutes_share) ≈ AdjEM` by construction — including it collapses the model to the player-impact identity, which kills the *swap-Δ* signal. For *projection* that identity is exactly what we want: `AdjEM_projected = f(Σ projected_cam_v3)`. The model becomes a clean calibrator and all projection error lives in the trajectory/freshman `cam_v3` predictions — honest and decomposable. Phase B's model consumes only what those models actually emit (`cam_v3`) plus structural features knowable for a projected roster (minutes, archetype, class) — no projected box-score lines, which is what forced `project_rotation`'s stat rescaling.
      - **v1 plan (in flight)**: new `training/train_roster_impact_model.py` trains on *actual* same-season `cam_gbpm_v3_psos` aggregates (4,255 team-seasons, 2015–2026) — a fast clean aggregator. Ships `roster_impact_model.onnx` + `roster_impact_model_meta.json` *alongside* the untouched box-score `roster_model.onnx`. Rust: new `cstat-core::roster_impact` feature module (`build_roster_impact_features`), `Predictor::predict_roster_impact{,_batch}`, `validate_roster_impact_meta` boot gate (`LoadError::RosterImpactMetaMismatch`). Acceptance: a new `cstat-ingest projections-backtest` subcommand runs the full composition pipeline for 2025/2026 with held-out OOF `cam_v3` inputs and must beat Phase A's 6.23 pipeline MAE — ideally with `PROJECTION_OFFSET` retired. **v2 follow-up** (named, not in this batch): retrain on held-out OOF `cam_v3` inputs ("train on what you serve") so the roster model itself absorbs the trajectory/freshman projection bias.
...
- **Top teams were systematically under-projected for 2027 (Phase A fix shipped 2026-05-20).** The projected roster handed to `predict_adj_em` carried every player's *prior* minutes with nobody promoted into vacated roles, so gutted rosters summed to ~150 player-minutes — out-of-distribution for a model trained on ~221 Σmpg. All 25 of 2026's top teams projected worse for 2027; Purdue (+36 AdjEM) scored at raw +3. Phase A (cam_v3-ranked rotation normalization + a backtest-calibrated baseline blend + probability-weighted floor/ceiling) cut the pipeline's 2025+2026 backtest MAE to 6.23, beating baseline-persistence (6.53); top teams now regress a modest ~1–5 AdjEM. The roster model remains a weak *projector* (raw MAE 9.97 vs baseline 6.53), so the blend is 80%-anchored to last year — Phase B (a cam_v3-aggregation projection model, not yet built) is the principled follow-up. → see "Projection bias fix — top teams systematically under-projected" in §5b.
