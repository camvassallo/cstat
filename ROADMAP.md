...
- [ ] **Projected impact at destination (Δteam rating)**: replace the raw 247 vs CamPom delta with a destination-aware projection. Two framings, sharing one projection engine.
  ...
  - [~] **Projection bias fix — top teams systematically under-projected** *(in progress, 2026-05-20)*. Spot-check of `/api/projections/2027`: **all 25** of 2026's top teams by AdjEM project *worse* for 2027; top-quartile mean Δ −7.66 vs bottom-quartile +0.65, league-wide −3.29. Root cause is **not** error in `predict_adj_em` — it's that `compose_all_projections` hands the model a roster that isn't a real roster. `ProjectedRoster::for_scenario` materializes every player at their *prior* minutes: returners at last year's role, recruits at a tier-fixed MPG, arrivals at their *source-team* MPG. Nobody is promoted into the minutes vacated by departed seniors / drafted players, so a gutted roster sums to ~150 player-minutes and a portal-stacked one past 230 — both well outside the ~221 Σmpg / ~6,100 `total_minutes` the roster model trained on (calibrated from 1,090 team-seasons, 2024–26). Purdue — a +36 AdjEM team — scored at **raw +3** purely because its projected roster summed to ~150 MPG. The freshman + trajectory models both emit `cam_v3`, which the box-score roster model never reads, so their per-player resolution is discarded. Fix lands in two phases:
    ...
    - [x] **Phase B — impact-aggregation projection model** *(shipped 2026-05-20)*. The box-score roster model was the wrong consumer for the `cam_v3`-emitting trajectory/freshman models. Phase B is a dedicated bottom-up model: features = aggregated *projected* `cam_v3` of the roster (Σ, minutes-weighted mean, top-1/3/7, counts over thresholds) + archetype balance + experience mix; target = `team_season_stats.adj_efficiency_margin`. At serve time it's fed projected `cam_v3` (trajectory for returners/arrivals, freshman for recruits). The box-score roster model is kept for the swap-Δ tool. Model-of-a-model by design — legitimate stacking, backtested end-to-end against actual 2025 + 2026 AdjEM.
      ...
      - **v2 follow-up** (named, not built): retrain `roster_impact_model` on held-out OOF `cam_v3` inputs ("train on what you serve") so the roster model itself absorbs the trajectory/freshman projection bias, and a LOSO-per-target roster model to remove the small in-sample leakage the backtest currently documents.
  - [ ] **Historical projection view — browse any season's preseason forecast**. Today `Projected2027.tsx` hard-codes the upcoming season (`fetchProjections(2027)`, the `/projected/2027` route, the title, `season=2027` deep-links). The backend already generalizes — `GET /api/projections/{year}` accepts 2025–2030 and composes from `year − 1`, so `/api/projections/2026` and `/2025` already return real forecasts (they're what the Phase A backtest ran against). Work is frontend-only: parameterize the page to `/projected/:year` with a year selector, so users can look at the forecast that was made going into each past season. **Valuable extension**: past years have *actual* results, so add an `actual_adj_em` field to the API response (target-season AdjEM; `None` for the live/future year) plus a "Projected vs Actual" column and a hit-rate / MAE summary — this turns the historical view into a *user-facing backtest surface* ("here's what we forecast for 2026, here's what happened"). Caveat: historical years have no `data/draft/{year}_early_entrants.json`, so their `?` cohort is empty (the route degrades gracefully — all declared players treated as returning; no floor/ceiling spread on past-year views).
...
- **Top teams were systematically under-projected for 2027 (Phase A fix 2026-05-20; Phase B shipped 2026-05-20).** The projected roster handed to `predict_adj_em` carried every player's *prior* minutes with nobody promoted into vacated roles, so gutted rosters summed to ~150 player-minutes — out-of-distribution for a model trained on ~221 Σmpg. All 25 of 2026's top teams projected worse for 2027; Purdue (+36 AdjEM) scored at raw +3. Phase A (cam_v3-ranked rotation normalization + a backtest-calibrated baseline blend + probability-weighted floor/ceiling) cut the pipeline's 2025+2026 backtest MAE to 6.23. **Phase B then replaced the box-score model entirely** with a dedicated impact-aggregation model that scores a roster from its *projected* cam_v3 distribution — raw MAE 6.58 / bias +0.44 (a genuine projector, not a descriptive model used out of context), blended pipeline MAE **5.88** at a 0.55 baseline weight with the `+2.0` offset retired. The residual error is now attributable to the upstream trajectory/freshman cam_v3 predictions rather than a roster-composition artifact. → see "Phase B — impact-aggregation projection model" in §5b.
