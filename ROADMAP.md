...
- [ ] **Predict page revamp** (UI redesign + backend + retrained model). Today's Predict is a thin wrapper over `GET /api/predict` that returns a margin and win%. Targeting a richer matchup view that's the de-facto destination for "what does the model say?", with the prediction sitting *inside* the comparison context rather than above it:
  - **UI**: lift PlayerCompare ergonomics — picker with team search, side-by-side header panels (record, AdjEM, CamPom leaders, archetype distribution, recent form). **Embed the Team Compare panel** from the §5b "Archetype-aware team views" item (radial roster plot + four-factor / roster-aggregate tables) directly under the headline margin + win% chip. Same data, two framings — the prediction says "Duke −3 vs Houston," the comparison panel explains *why*.
  - **Explainability** (subsumes the previously-standalone "Game prediction explainability" bullet): per-feature attribution surfaced as a "Keys to the Game" panel — synthesised narrative bullets ranking which model-weighted groups of stats are pushing the spread, plus a Side-by-Side stats panel and per-team Four-Factors tug-of-war bars. Currently uses **ablation** for the per-feature attribution that drives the keys panel (zero each feature, measure the prediction shift) — pure-Rust, no new deps, but produces wrong-direction sign artifacts when features interact strongly. The keys panel works around this by taking *direction* (which team has the edge) from the raw stat sign × `homeAdvantageSign` lookup (`web/src/components/featureExplanations.ts`), while keeping *importance* (how much it matters) from the model's |contribution|. Followup: **TreeSHAP in pure Rust** for proper interaction-aware attribution. Two paths: (a) parse the LightGBM `.lgb` text dump (~1k–2k lines per tree, well-documented format) and run TreeSHAP against it — needs the `.lgb` file shipped alongside the existing `.onnx` (small, ~150KB) ; (b) extract the tree structure from the existing ONNX `TreeEnsembleRegressor` node (more compact but less documented). Either keeps the API in Rust — no LightGBM C dep, no Python sidecar. Once TreeSHAP is in, drop the `homeAdvantageSign` workaround in `featureExplanations.ts` and the model's signed contribution becomes the authoritative direction.
  - **API**: extend `GET /api/predict` (or add `POST /api/predict` accepting `{home, away, neutral, season}`) to return margin, win%, top-N SHAP contributions, and the data the embedded Team Compare needs in one round-trip. Reuse the existing `features.rs::build_game_features` so the on-the-wire and trained features stay aligned.
  - **Recent / upcoming matchups panel**: list of recent and near-future games from the schedule with one-line predictions; click-through to the Previous Matchup view (completed) or to Predict pre-loaded with the matchup (upcoming). Pairs with the §5b Previous Matchup view to make the prediction-page the central matchup hub.
  - **Model retrain on 2024+2025+2026**: with 2024 ingested, retrain LightGBM on the combined three-season cohort. Phase 6 calls this out as the highest-leverage improvement available — current model early-stops at 49-66 iterations on two seasons, data-starved. Pre-req cleared in this PR (`crates/cstat-core/src/compute.rs` and `training/features.py` AST% formulas now source `tgs.fgm` so 2024's missing `pgs.team_fgm` doesn't NaN out the rate stats). Sanity-check for any other 2024-only data shape diff before training; ship the new ONNX model with the revamp.
  - **Acceptance**: every existing predict use case still works (back-test, head-to-head); SHAP panel renders for any matchup; Team Compare panel matches the standalone Team Compare on the same matchup; backtest MAE / win acc / AUC strictly improve over the current 2025+2026-only model on a held-out test set.
