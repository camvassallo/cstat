...
- [ ] **Preseason × pit blend for honest early-season predictions** *(captured 2026-05-29 after pit smoke testing)* — the audit's R5 wiring shipped, but spot-checks against real 2026 games exposed the next layer: very-early-season games where `torvik_player_game_stats` is essentially empty produce *correctly honest* but *useless* predictions (Duke vs Army 2025-11-11 honest-projects at +1.5; actual was +55, because on 2025-11-10 there were ~0 per-game Torvik rows for the season). Pit predictions converge to sharpness by mid-January (Duke vs Wake Forest 2026-01-24: pred +22.2 vs actual +21), but the first ~4 weeks of every season are the regime where honesty alone isn't enough — we need a prior to anchor on while the in-season signal accrues.
  ...
  **Three nested investigations**, sequenced by dependency. Don't skip (1) — blending bad preseason into an honest pit feed makes the in-season prediction *worse*.
  ...
  1. **Preseason projection accuracy audit** *(prerequisite)*
     - **Question**: how well did `/api/projections/2026` (Phase B impact-aggregation model, computed pre-season 2026) predict actual 2026 AdjEMs? Phase B's documented LOSO-blended pipeline MAE 5.86 vs baseline-persistence 6.53 says the model adds ~0.67 MAE over "last year again," but a single pooled number hides per-team error structure.
     - **Per-team error distribution**: where is the "sticky last-year" assumption strongest? Specifically: do teams with high turnover (departing seniors + drafted players + outgoing portal) project *less* accurately than continuity-heavy teams? If so, the trajectory model is being asked to project minutes that don't exist on the projected roster (the §5b Phase A rotation-normalization caveat).
     - **Residual signal hunt**: regress projection error against (a) coaching changes, (b) recruiting class composite (avg star rating, # top-100), (c) returning-minutes fraction, (d) net portal Δ-CamPom. If any of these systematically predict the residual, they're missing-feature candidates for the Phase B model — see (2) below.
     - **Honest framing**: same elite-tail regression bias the trajectory model carries (§5c "Trajectory projections for elite returners are extrapolation"). Phase B's calibrator absorbs the systematic component; the per-team residual is what (1) is measuring.
     - **Deliverable**: `training/audit_preseason_2026.py` + a one-page findings doc in `eval_history/`. No DB or schema changes; pure measurement.

  2. **Preseason projection improvements** *(blocked on (1) — make sure we're fixing the right thing)*
     - **Candidates ordered by expected lift, contingent on (1)'s findings**:
       - **Coaching-change indicator** — turnover at the head-coach level is plausibly the largest non-roster predictor of variance. Data sourced from a curated JSON or a Wikipedia-scraped list (no clean API exists). Per-team flag for "new HC this offseason"; consider a "first-year-at-program" flag separately.
       - **Team-level recruiting class composite** — currently the freshman impact prior projects per-recruit cam_v3, then the roster-impact model aggregates. Adding the class-level composite (avg star, # top-100, # top-30) as its own roster feature could surface "this team's incoming class is unusually strong/weak" signal the per-recruit projection averages away.
       - **Returning-experience weight** — Phase B already has minutes-share returning as a structural feature. Worth checking whether the model is using it productively (per-feature SHAP), and whether a non-linear interaction (high turnover × high returning-cam_v3 = elite returners on a gutted roster) is being missed.
       - **Portfolio-style portal Δ** — currently arrivals get projected via the freshman/trajectory pipelines, but a team-level "net portal +/−" feature would let the model down-weight teams with churn-heavy roster construction independent of who specifically came/went.
