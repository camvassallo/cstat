...
- [~] **Freshman recruiting ingest + tier-mean heuristic + per-player prior model**. The /projections page now consumes a third roster source. **Ingest shipped: 1,200 recruits across class-of-2024/2025/2026 (321 + 512 + 367). Tier-mean heuristic shipped: 558 of those recruits paired to actual freshman cstat-seasons (224 from 2024 → cstat-season 2025 freshmen; 334 from 2025 → cstat-season 2026 freshmen; class-of-2026 hasn't played yet) → 4-tier population profile keyed on composite_rank. Per-player prior model deferred.**
  ...
  **Surface trajectory + freshman projections everywhere — 3-PR plan.** Driven by an observation (Q1 on regression-to-the-mean): the trajectory model systematically projects elite returners *lower* than their current-season CamPom — Boozer 2026 +30.54 → 2027 projected +15.93 (Δ −14.61). This is real signal (empirical regression-to-the-mean + selection bias on returners who stay + thin tail support in training data), but it'll surface as confusing negative Δs as soon as we put projected CamPom on Transfer and Projection surfaces. PR 1 lays the framing, PR 2 and PR 3 propagate the chip pattern from the Recruits tab to the rest of the site.
  ...
  - [ ] **← UP NEXT: PR 2 — Projected CamPom + Δ on Transfer page.** New `Predictor::predict_trajectory_batch(rows: &[[f32; TRAJECTORY_NUM_FEATURES]]) -> Result<Vec<TrajectoryPrediction>, ort::Error>` mirroring `predict_freshman_batch` (single contiguous tensor per ONNX session, empty-input short-circuit, output-length validation). Extend `/api/transfers/{year}` to fetch each transfer's prior-season trajectory features in one batch SQL query (mirroring `fetch_player_trajectory_row` but over N rows), build feature vectors, batch-predict, surface `projected_campom_{mean,lower,upper}`. Frontend `TransferPortal.tsx` gains a `Proj` column (CamPom-tier-colored chip) and a `ΔCP` column (`projected − current`, color-coded — **negative=red is the regression-typical case** for elite transfers). Rename existing `Δ` column to `Δ247` to disambiguate from the new `ΔCP`. Tooltip: "Trajectory model is destination-agnostic — projection assumes role similar to source team. Real destination role may differ." New `trajectory_batch_matches_single_row` + `trajectory_batch_empty_input` tests. Transfers without `cstat_player_id` (unresolved) skip prediction → NULL projection.
  - [ ] **PR 3 — Per-player projections on 2027 projection page.** Extend `compose_all_projections` (or a helper) to compute per-player projected CamPom for every roster slot: returners → trajectory model on their 2026 stats; arrivals (incoming transfers) → trajectory model on source-season stats; recruits → freshman model (already wired into `synthesize_freshman_row` as of the swap above). Route response shape change — `ProjectedRoster` gains per-player `projected_campom_{mean,lower,upper}` + `current_campom` + role (`returning` / `arrival` / `recruit` / `uncertain`) + identity fields (name, player_id). Frontend team-detail card in `Projected2027.tsx` (or its sub-component) renders projected CamPom chip per roster row with current-vs-projected delta on hover; group by role with separators. **Optional within scope**: "Top projected CamPom" summary column on the main projections table (replaces or augments base-season leader); defer to a follow-up if PR 3 gets unwieldy. Biggest route-shape change of the three, so lands last; both PR 2 and PR 3 use the same `predict_trajectory_batch` so PR 2 settles that pattern first.
