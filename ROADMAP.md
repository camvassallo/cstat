...
- [x] **Predict page revamp** — shipped in PR #43. The page is now the de-facto matchup destination: side-by-side header panels (record, AdjEM, CamPom leaders, archetype distribution, recent form), 3-way venue picker (home / away / neutral) with symmetric neutral-site predictions, big margin + win% chip, "Keys to the Game" narrative bullets driven by per-feature **ablation** in `crates/cstat-core/src/inference.rs::predict_with_contributions` (`PredictionResult.contributions[i]` = `full_pred − pred_with_feature_i_zeroed`), Side-by-Side stats panel, and per-team Four-Factors tug-of-war bars. Backend: `POST /api/predict` accepts `{home, away, neutral, season}` and returns margin + win% + top contributions in one round-trip; reuses `features.rs::build_game_features` so on-the-wire and trained features stay aligned. Model retrained on 2024+2025+2026 (margin model 157KB → 353KB; metrics in `training/models/model_meta.json`). The keys panel currently takes *direction* (which team has the edge) from a hand-coded `homeAdvantageSign` lookup in `web/src/components/featureExplanations.ts` while keeping *importance* (how much it matters) from the model's |contribution|. Carved-out follow-ups below.
- [ ] **Predict follow-up — TreeSHAP in pure Rust** (replaces the ablation-based attribution). Ablation produces wrong-direction sign artifacts when features interact strongly, which is why the keys panel currently leans on the `homeAdvantageSign` workaround. Two paths from `crates/cstat-core/src/inference.rs:201`: (a) parse the LightGBM `.lgb` text dump (~1k–2k lines per tree, well-documented format) and run TreeSHAP against it — needs the `.lgb` file shipped alongside the existing `.onnx` (~150KB); (b) extract the tree structure from the existing ONNX `TreeEnsembleRegressor` node (more compact but less documented). Either keeps the API in Rust — no LightGBM C dep, no Python sidecar. Once TreeSHAP is in, drop the `homeAdvantageSign` workaround and the model's signed contribution becomes the authoritative direction.
- [ ] **Predict follow-up — totals / tempo model for final scores**: today the Predict page (and TeamDetail schedule's Projected column, and the score ticker) renders predictions as a margin (`Duke −3.2 · 64%`). To show actual final scores like `75-72` the way KenPom and Vegas do, we need a totals/tempo model alongside the existing margin model — predict total points (or possessions × eff), then derive the two team scores from `total ± margin`. New training pipeline (similar feature set, regression target = home + away pts), new ONNX export, surface the score everywhere current predictions render. Starting point: the existing `tempo` and per-team adj_offense / adj_defense features already encode most of the signal — start with a quick LightGBM regression and see how far it gets before considering more sophisticated tempo modeling.
- [ ] **Predict follow-up — point-in-time historical predictions** (retroactive `game_forecasts`): today the Projected column always reflects "what we'd predict now" using current-state team features, which is slightly odd on completed games (we already know the result). For completed games, render "we predicted X / actual was Y" using only data available *before* tip-off. The training pipeline already builds point-in-time features per game (`features.py`), so the work is: (1) batch-run the trained model against every historical game with its pre-game feature snapshot, (2) store predictions in the existing `game_forecasts` table (already used for NatStat forecasts — extend with cstat columns or a `source` discriminator), (3) plumb to the schedule/ticker UI so completed rows become an accuracy receipt. Bonus: enables a calibration dashboard (§6 "Model accuracy dashboard with calibration tracking") and unblocks the totals model's training validation.
