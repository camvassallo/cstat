...
- [~] **Post-12-season backfill follow-ups (next session)** — the 2026-05-17 backfill brought us from 5 → 12 ingested seasons (2015–2026) in one sitting. Several downstream pieces became unlockable but weren't done in-session; queueing them here so they don't drift.
  ...
  - [x] **Freshman model retrain** (Shipped 2026-05-17) — no SEASONS constant, so corpus auto-rebuilds from `recruits.cstat_player_id` joins. Re-resolve pass (item below) found 0 additional joins for 2021–2024 (the unresolved tail is genuinely non-D1 recruits), so corpus stayed at **1,154 rows**. **LOCO pooled MAE 2.477 → 2.462** (+0.015, essentially noise from a stochastic re-fit on identical data); 5-fold CV 2.439 → 2.433. Beats baseline by 2.9% (was 3.9% — the slip is fold-noise, not regression). 1,154 OOF predictions persisted; boot gate passes. Real lift here is gated on pre-2021 recruit-class ingest, not retraining.
  ...
  - [~] **Re-resolve transfer + recruit player IDs for 2021–2024** (Partially shipped 2026-05-17) — used the new `--resolve-only` flag (see Refactor Backlog). **Transfers: big win** — 2021 went 0/621 → **545/621 (87.8%)**, 2022 went 0/734 → **642/734 (87.5%)**, 2023/2024 idempotent at ~86% (already resolved). **+1,187 newly-resolved transfer rows** flow into the trajectory model's older-class training mass. **Recruits: no change** — 2021–2024 were already at their ceiling (~70–76%) when first ingested (destination season was already loaded by the time recruits got ingested, unlike transfers). The unresolved tail (54–82 rows/year) all logged `tier1_exact=0 tier2_nickname=0` — genuinely non-D1 recruits (JUCO/prep path, name beyond nickname matcher, never played D1), not a destination-not-loaded problem. Pre-2021 recruit-class ingest is the actual unlock for freshman-model coverage.
...
- **Freshman-impact prior model: 3.9% pooled LOCO lift vs tier-mean baseline.** Wins on every held-out class (2021 → 2022 best, +0.201 MAE; 2023 → 2024 worst, +0.013). Leave-one-class-out CV (every class held out, retrained on the rest, scored on held-out) gives pooled MAE 2.477 vs weighted-LOCO tier-mean baseline 2.578 (+0.101 / +3.9%); the training script reports a smaller 2.3% delta because it compares LOCO against the *in-sample* tier-mean baseline (MAE 2.535) rather than the per-fold-train-only baseline. Gap to 5-fold random CV (2.439) is only +0.038 — no severe overfit, mild fold-overlap leakage. Corpus expanded from 963 → 1,154 rows when class-of-2021 ingested 2026-05-15; pre-2021 recruit backfill would add another fold but the next-class-down's coverage / data quality on 247 historicals are the binding constraint. Selection bias on top recruits who returned for sophomore year is sharper here than for trajectory: elite freshmen leave, so the model is calibrated on a thinner cohort than headline MAE implies. → see "Freshman-impact prior model" in §6.
