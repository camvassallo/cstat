...
- [ ] **Predict page revamp** (UI redesign + backend + retrained model). Today's Predict is a thin wrapper over `GET /api/predict` that returns a margin and win%. Targeting a richer matchup view that's the de-facto destination for "what does the model say?", with the prediction sitting *inside* the comparison context rather than above it:
  ...
  - **Explainability** (subsumes the previously-standalone "Game prediction explainability" bullet): per-feature attribution surfaced as a "Keys to the Game" panel + per-feature info tooltips. Currently uses **ablation** (zero each feature, measure the prediction shift) — pure-Rust, no new deps, but produces wrong-direction sign artifacts when features interact strongly. The keys panel works around this by taking *direction* (which team has the edge) from the raw stat sign × `homeAdvantageSign` lookup, while keeping *importance* (how much it matters) from the model's |contribution|. Followup: **TreeSHAP in pure Rust** for proper interaction-aware attribution. Two paths: (a) parse the LightGBM `.lgb` text dump (~1k–2k lines per tree, well-documented format) and run TreeSHAP against it; (b) extract the tree structure from the existing ONNX `TreeEnsembleRegressor` node (more compact but less documented). Either keeps the API in Rust — no LightGBM C dep, no Python sidecar. Once TreeSHAP is in, drop the `homeAdvantageSign` workaround and the model's signed contribution becomes the authoritative direction.
