  ...
  **Three nested investigations**, sequenced by dependency. Don't skip (1) — blending bad preseason into an honest pit feed makes the in-season prediction *worse*.
  ...
  2. **Preseason projection improvements** *(blocked on (1) — re-prioritized after audit findings)*
     - **Candidates ordered by audit-supported expected lift**:
       - [ ] **PR A — Net portal Δ-CamPom as a roster-impact feature** *(highest-confidence; OLS-significant)*. Add `outbound_cam_v3_sum` (and/or `delta_vs_baseline_roster_cam_v3`) to the Phase B roster-impact feature set. Retrain on the 12-season cohort; rerun `audit_preseason_projections.py`. **Acceptance**: Q4 bias closes by ≥1.5 AdjEM, pooled MAE drops by ≥0.10.
