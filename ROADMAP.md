...
- **Freshman-impact prior model wants ≥5 paired classes; we have 2.** Soft-blocked on Phase 6 historical data backfill. Recruit ingest keeps running while we wait. The shared recruit-feature extractor is already in place from Phase 5c-iter2, so the freshman model build is feature-pipeline-ready. → see "Freshman-impact prior model" in §6.
