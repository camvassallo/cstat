...
- [~] **Roster fit scoring built on archetypes**: redundancy detection ("team has 3 Sorcerers, missing a Cleric") layered on top of the Identity/Gaps index already shipped on TeamDetail. Score each portal player at each plausible destination by how much they fill a Gap vs add to an Identity stack.
  - [x] **v1 — per-transfer Fit column on TransferPortal** *(shipped 2026-05-26)*. New `crates/cstat-core/src/roster_fit.rs` exports `compute_fit_score(primary, secondary, team_dist) -> FitScore` — pure function over the existing `ArchetypeShare` shape, no DB dependency. Per-class signal: `index = 0` (totally missing) → +1.0; `index = 1.0` (D-I average) → 0.0; `index = 2.5+` (saturated redundancy) → −1.0. Primary 1.0× + secondary 0.5× weighting matches the Identity/Gaps SQL; combined raw lives in `[-1, +1]` with five tiers (`StrongFit > 0.40 > GoodFit > 0.10 > Neutral > -0.10 > SomeRedundancy > -0.40 > Redundant`). Bulk SQL helper `queries::get_archetype_distributions_for_teams(team_ids, season)` mirrors `get_team_archetype_index` in one round-trip across hundreds of destinations. Transfers route fetches once for unique `next_team_id`s and injects `fit_score` / `fit_tier` / `fit_label` / `fit_primary_index` per row. Frontend renders a tier-tinted chip ("Fills Cleric gap", "Stacks Wizard rotation", "Roster-neutral") with the raw value + destination index in the tooltip; sortable column. **Live distribution on /api/transfers/2026** (746 scored / 1,505 total): mean 0.04, median 0.00, 4.2% saturated at the strong-fit end (rare-class gaps like a Fighter joining a team with zero Fighters), 0.7% at saturated-redundancy. Most rows (~66%) land in the mid-tier informative zone (some redundancy, neutral, good fit). Baseline is destination's **source-season** archetype distribution — same numbers feeding TeamDetail's Identity/Gaps panel; doesn't account for next-season turnover (graduating seniors, outbound portal). 10 unit tests cover the signal math (saturation, neutrality, secondary half-weight, secondary-matches-primary dedup, empty distribution, NaN guards) plus the label-routing rules.
  - [ ] **v2 — fit-adjusted Δrating + projected-roster baseline**. The roadmap's "Combine with the impact projection above for a final 'fit-adjusted Δrating'" plus a baseline upgrade to the projected next-season roster (currently uses source-season, which doesn't account for who else is arriving / departing). Recruits-have-no-archetype is the load-bearing blocker on the projected baseline: synthesized freshman PlayerRows carry `primary_class: None`, so any projected-distribution that ignores them biases against teams with big incoming HS classes. Options: (a) infer freshman archetype from recruit features (height/position/rank → empirical archetype distribution), (b) carry archetype uncertainty through to the fit score with a wider band. Defer until there's a product signal that v1's framing is insufficient.
