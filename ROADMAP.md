...
- [~] **Post-12-season backfill follow-ups (next session)** — the 2026-05-17 backfill brought us from 5 → 12 ingested seasons (2015–2026) in one sitting. Several downstream pieces became unlockable but weren't done in-session; queueing them here so they don't drift.
  ...
  - [ ] **Le Moyne 2026 NatStat coverage gap** — NatStat returns `NO_DATA` for `team/mbb/LEMN/2026` despite Le Moyne playing a full 15-15 D-I season in NEC (third year of transition; Wikipedia confirms). Scanned all 60 pages of `teamperfs/mbb/2026` (6,000+ records) — zero LEMN entries. Even opponent schedules (e.g., CCSU's 33 perfs) omit the Le Moyne games. NatStat appears to filter transition-period teams from D-I stats — same pattern would hit any future transitioning program in their final-but-not-tournament-eligible year. No cstat-side fix; the Le Moyne 2026 ghost row is now deleted along with the others. If NatStat backfills retroactively, the next `season --year 2026` ingest will recreate the row cleanly. Open question: do we want a per-season "known coverage gaps" allowlist that surfaces this honestly in the UI, or is silent omission OK?
