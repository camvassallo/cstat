...
- [ ] **Predict follow-up — embedded Team Compare panel**: lift the radial roster plot + four-factor / roster-aggregate tables from §5b's "Archetype-aware team views & Team Compare" item directly under the headline margin + win% chip. Same data, two framings — the prediction says "Duke −3 vs Houston," the comparison panel explains *why*. Depends on §5b shipping the radial-plot building block first.
- [ ] **Predict follow-up — recent / upcoming matchups panel**: list of recent and near-future games from the schedule with one-line predictions; click-through to the §5b Previous Matchup view (completed) or to Predict pre-loaded with the matchup (upcoming). Same surface as the §4b "Score ticker / recent results" item — the two converge: the score ticker is the global strip, this is the Predict-page-scoped panel that pairs with the matchup the user is actively viewing.
