...
- [ ] **Returning-player growth model**. Train a LightGBM regressor on `(season N stats) → (season N+1 CamPom v3)` pairs across every consecutive-season player in the DB. Inputs: prior-season rate stats, archetype, minutes share, class year (freshman→sophomore is the steepest growth bucket; senior→grad-transfer flattens), prior-season Torvik composite, **recruit-rank features** (see below). Output: predicted next-season CamPom v3 (and a confidence band from quantile regression). Sanity-check honest framing: ~3 seasons of paired data → ~10k returning-player rows after gating on min-games qualified in both years, enough for a baseline but the per-class-year sample is thin and the model should not be over-sold. **Acceptance**: MAE per class-year bucket better than naive "year N+1 ≈ year N" baseline; for transferring players, factor in destination-team archetype mix to avoid systematically over- or under-projecting role changes.
  ...
  **Recruit-rank as a tested feature** (not a foregone-conclusion lift). Conventional analyst wisdom is that highly-ranked recruits show steeper year-over-year growth — rank reflects scout consensus on ceiling/tools/IQ that may not yet be expressed in freshman box-score output. The 5c model as scoped above implicitly claims next-year value can be inferred entirely from observed performance; adding rank *tests* whether scout consensus adds signal beyond that. Two outcomes, both useful: (a) rank lifts out-of-sample MAE → confirms conventional wisdom and the model gets better; (b) rank doesn't lift MAE → publishable finding ("once a player has a season of college minutes, pre-college rank adds nothing"). The data alignment is the cstat-specific edge — most shops don't have rank-at-signing joined to college box scores; cstat will via `recruits.cstat_player_id` (see Phase 6 recruit-ingest bullet).

  **Feature shape**:
  - `composite_rank` (INT, NULL for unranked) — 247 composite rank at signing.
  - `years_since_recruit` (INT, NULL for unranked) — how stale the rank is. Encodes decay: rank should matter most for Fr→So and fade through Sr.
  - `is_ranked` (bool) — explicit unranked-cohort flag so the model fits a separate baseline for the modal case. Most cstat returners are unranked (walk-ons, JUCO transfers, internationals, low-major recruits below 247's cutoff); without this flag, NULL imputation muddies the majority-cohort fit.

  **Redundancy note**: `years_since_recruit` and `class_year` are perfectly correlated for the standard path (HS-ranked recruit → linear college progression: Fr=0, So=1, …). LightGBM handles redundancy fine — picks one per tree split. Keep both anyway because they *diverge* in informative off-path cases: JUCO transfers with HS rank, reclassifiers, gap-year prospects. For the modal aligned case, feature importance will favor whichever the trees split on first; that's the model's call.

  **Caveats — load-bearing, frame outputs honestly**:
  - **Selection bias on returners.** Top-ranked recruits who *return for sophomore year* are negatively selected — the Cooper Flagg / Boozer cohort leaves for the draft; the 5-stars who stay are disproportionately the ones whose freshman year disappointed. Mid-tier returners are average. A naive "rank → growth" model trained on returners may find the opposite of conventional wisdom without that meaning anything. **Diagnostic**: report MAE on full cohort vs. "stayed past year 1" cohort separately; surface the gap. If the rank coefficient flips sign on the stayed-only cohort, that's selection talking, not signal.
  - **Sample size below the top**. Rank-as-feature signal is real at the top (Five-Star → All-American is reliable) and noise below ~30. With ~3 seasons of paired data, the top-30 ranked-and-returning cohort is tens of players, not thousands. Don't claim more precision than the cohort supports.
  - **Unranked is the modal case**. Most cstat returning-player rows have no 247 rank. The model needs to fit this cohort *well* (it's the majority of training data); the `is_ranked` flag is how it does that.

  **Dependency**: needs the Phase 6 recruit-ingest bullet to land first (rank lives in the new `recruits` table). The ingest is the prereq — not the freshman-impact prior model, which is a separate downstream consumer of the same table.
...
- [ ] **Freshman recruiting ingest + freshman-impact prior** (closes the last gap in the Projected next-season roster surface). Today's `/projections` page renders rosters from returners + incoming transfers only; a top-5 recruit landing at Duke moves the projection by zero. This bullet adds the third source.

  **Why it's parked under Phase 6, not 5b**: building a credible recruit-rank → freshman-CamPom prior needs 5+ paired classes of (rank at signing, actual freshman season). We currently have 2 cstat seasons ingested. The model is data-starved until the historical-data backfill above runs, so the ingest pipeline can ship now but the prior model should follow the backfill.

  **Data source — 247Sports composite HS recruit rankings**. URL pattern, params, and auth captured in auto-memory (`reference_247sports_recruits.md`): `https://247sports.com/season/{YEAR}-basketball/compositerecruitrankings/?ViewPath=…&InstitutionGroup=highschool&Page=N`, gzipped HTML fragment ~15KB/page, JWT cookie auth (the existing `TFS_247_JWT` env var works; `GUEST_JWT` is auto-issued for the public ranking pages). Iterate `Page` until empty (~4-5 pages of ranked players per class year). Cross-reference against On3 / Rivals if cross-source consensus is useful — same multi-source pattern as the draft big board (different scout per source field).

  **Migration**: new `recruits` table — `(year, recruit_key)` UNIQUE on 247's stable player key, `name`, `full_name` STORED, `composite_rank`, `composite_rating`, `stars`, `position`, `height`, `weight`, `hometown`, `high_school`, `committed_team_id UUID NULL REFERENCES teams(id)`, `committed_team_natstat_id` for cross-season joins, `institution_group` TEXT + CHECK constraint (`highschool` / `juco` / `prep`, enum-via-CHECK same as transfers so widening is a one-line migration), `raw_player JSONB`, `last_update_date`, `cstat_player_id UUID NULL REFERENCES players(id)`. Index set mirrors `transfers`: `(year, composite_rank)`, `(year, committed_team_id)`, `lower(full_name)`, partial `cstat_player_id` reverse-join.

  **Client + ingest**: reuse `TfsClient` JWT plumbing + rate limiter from `crates/cstat-ingest/src/tfs.rs`. HTML parsing is the new piece — add `scraper` crate, build a row extractor (rank, name, school commit, height/weight, position, composite rating/stars). New `crates/cstat-ingest/src/ingest/recruits.rs` with `ingest_live(client, pool, year)` (paginate until empty), `bootstrap_from_snapshot(pool, year, path)` (offline seed from `data/recruits/{year}_raw.json` — gitignored, same pattern as `data/transfers/{year}_raw.json`), `resolve_cstat_joins(pool, year)` post-ingest pass.

  **Two-pass join strategy** (recruits don't exist in `players` at ingest time — they're still in high school):
  - **Pass 1 (ingest-time)**: `(committed_school)` → `teams.id` via the alias table built for transfers. Sets `committed_team_id`. Run on every ingest.
  - **Pass 2 (post-arrival)**: case-insensitive `(name, committed_team_id, freshman_cstat_season)` → `players.id`. Runs after the cstat-season ingest auto-creates the player rows from box scores. For class-of-2026 recruits, Pass 2 fires after cstat-season 2027 box-score ingest. Sets `cstat_player_id`.

  **CLI**: `cstat-ingest recruits --year YYYY [--bootstrap-from PATH] [--no-resolve-teams]`. **Year semantics**: `year` is the *recruiting class year* (spring of HS graduation, matches 247's URL convention). Recruits for `year=2026` first appear in box scores during *cstat-season 2027* — same offset-by-one as the transfer portal (also already documented in 5b).

  **API + route**: `GET /api/recruits/{year}` returns the resolved list joined to `teams` for the destination chip. Shape mirrors `crates/cstat-api/src/routes/transfers.rs` — one row per recruit with rank, school, position, projected freshman CamPom (from the prior model below) + uncertainty band.

  **Freshman-impact prior model** (the load-bearing modeling piece):
  - **Training pairs**: historical (composite rank, position, height/weight, committed-school AdjEM tier) → actual freshman-season CamPom v3. Needs ≥5 paired classes for a credible model; coupling to the historical-data backfill is what makes the Phase-6 placement honest.
  - **Inputs**: composite rank, composite rating, position, height/weight, **committed-school tier** (top-25 AdjEM destinations get a usage/minutes premium that freshmen at low-major schools don't see — load-bearing feature).
  - **Output**: predicted freshman CamPom v3 + quantile-regression bands. Wider bands than the 5c growth model because freshmen have no prior box-score signal.
  - **Honesty framing**: recruit-rank → impact correlation is strong at the top (Five-Star → All-American is reliable) and noise below ~30th-ranked. Bands need to widen accordingly; ranked-but-not-elite recruits render as a wide range, not a point estimate.
  - **Train/serve contract**: same pattern as `roster_model_meta.json` — `recruit_prior_meta.json` lists features, qualification gate, and classes trained on; Rust inference path hard-fails on drift at boot.

  **Plug into Projected next-season (5b)**: once recruits + prior land, `crates/cstat-api/src/routes/projections.rs` gains a third roster source — returners + incoming transfers + **incoming recruits with projected freshman lines**. Floor/ceiling rendering extends naturally: high-uncertainty freshman projections widen the band the same way the `?` cohort (early-entrants undecided pre-deadline) already does.

  **Optional surface — standalone `/recruits` page**: mirrors TransferPortal — sortable AG Grid with Rank · Player · Position · Stars · Committed School · Projected Freshman CamPom · Δ from composite rank. Same value-delta framing as the portal (positive Δ = prior model thinks recruit outperforms their composite). Lower priority than the projections integration above.

  **Dependencies + sequencing**:
  - **Ingest is now a prereq for Phase 5c's growth model**, not parallel to it. 5c tests recruit-rank as a feature (`composite_rank`, `years_since_recruit`, `is_ranked`); rank lives in the `recruits` table; ingest needs to land first. This reverses the earlier "5c first, recruits later" ordering.
  - The *prior model* (year-0 → year-1 freshman projection) still has the historical-data backfill as a soft prereq — credible training needs 5+ paired classes, and we have 2 cstat seasons. Ingest can ship now and accumulate snapshots while the backfill runs.
  - Phase 5c is the natural neighbor for "year 1 → year 2 onward"; this bullet covers "year 0 → year 1" for the same player. Together they project any player's next-season line regardless of where they are in their career arc.
  - **Two-stage value capture**: ingest unlocks 5c's rank feature immediately (the recruit table is read by 5c training, not by users); the prior model + standalone `/recruits` surface follow once paired classes accumulate.
