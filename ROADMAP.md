  ...
  **Three nested investigations**, sequenced by dependency. Don't skip (1) — blending bad preseason into an honest pit feed makes the in-season prediction *worse*.
  ...
  2. **Preseason projection improvements** *(blocked on (1) — re-prioritized after audit findings)*
     - **Candidates ordered by audit-supported expected lift**:
       ...
       - [ ] **PR B — Trajectory model regression-to-mean on thin rosters** *(re-prioritized — bumped to head of queue by PR A's decomposition)*. The decomposition pinned Q1 over-projection at **+5.62 upstream bias** vs +2.69 calibrator — when a team loses 60%+ of cam_v3 (Maryland 2026 lost +38.6 cam_v3, replaced only +9.6, finished at +5.2 vs preseason +21.6), the trajectory model projects the remaining returners closer to their class-year-archetype average than to their actual modest current selves. **Acceptance**: Q1 upstream bias closes by ≥2 AdjEM. **Approach options**: (a) condition trajectory predictions on a "roster-attrition" feature (Σ departed cam_v3 / Σ team cam_v3 base season) so the model can learn "thin-roster returners regress less aggressively"; (b) retrain trajectory on a residualized target (residual against same-team prior) to break the team-prior signal that overweights "they were good last year" for thin rosters; (c) add an explicit shrinkage factor toward prior season's own cam_v3 (vs the model's class-year-archetype mean) that's higher when team continuity is low. Decompose trajectory OOF bias by (prior-team-attrition-quartile, prior-campom-bucket) to confirm the mechanism before picking an approach.
       - [ ] **PR C — Roster-impact calibrator top-tail compression** *(new — surfaced by decomposition)*. Q4 top-quartile calibrator bias is **−2.49 even given perfect inputs** (oracle roster + actual cam_v3) — the model itself compresses the upside. Michigan 2026 oracle was +35.4 vs actual +44.6 (model can't extrapolate to that ceiling). **Approach options**: (a) re-evaluate the `ROSTER_INCLUDE_IMPACT=1` variant that was originally rejected for the swap-Δ tool — for the *projection* aggregation use case the player-impact identity is exactly what we want; the box-score model's 1.68 MAE / R² 0.98 wasn't a bug, it's the right calibrator; (b) monotonic constraint on `cam_sum` (LightGBM `monotone_constraints`) so the model can't shrink toward the center on high-cam_sum rosters; (c) per-tail quantile-targeted loss (Huber with asymmetric clipping). Worth quantifying whether the box-score model's R²=0.98 variant beats the diff-feature one on the decomposition's Q4 calibrator bias before picking an approach.
