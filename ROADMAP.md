...
- [ ] **Predict follow-up — totals / tempo model for final scores** *(training pipeline shipped; Rust + API + UI deferred)*: today the Predict page (and TeamDetail schedule's Projected column, and the score ticker) renders predictions as a margin (`Duke −3.2 · 64%`). The end goal: show final scores like `75-72` the way KenPom and Vegas do, deriving the two team scores from `(total ± margin) / 2`.
  - **Done in the training pipeline** (`training/`): `total` target added to `features.py`; `train_total_model` + 5-fold CV + chronological backtest + final-on-all-data fit added to `train.py`; new `total_model.lgb` saved alongside `margin_model.lgb` and `win_model.lgb`. **Critical lesson learned**: the existing 49 `diff_*` features (home minus away) cannot predict totals — diffs throw away absolute level. `diff_tempo=0` is ambiguous between two slow teams (low total) and two fast teams (high total). Initial training on diffs alone gave MAE 15.2 / R² −0.009 (worse than predicting the league mean). Added 9 level-sensitive `sum_*` companion features (`sum_adj_tempo`, `sum_adj_offense`, `sum_adj_defense`, `sum_effective_fg_pct`, `sum_opp_effective_fg_pct`, `sum_w_ppg`, `sum_w_ortg`, `sum_off_rebound_pct`, `sum_def_rebound_pct`) which now occupy 6 of the top 7 spots in totals model feature importance. Result: backtest MAE 13.58 / R² 0.179. Margin/win models stay on the unchanged 49-feature diff matrix — `model_meta.json` carries separate `features` (49, margin/win) and `total_features` (58, total) lists so the existing Rust `Predictor` (NUM_FEATURES=49) is unaffected.
  - **Deferred work to actually ship projected scores**:
    - **`export_onnx.py`**: extend to also emit `total_model.onnx` (read `total_n_features` + `total_features` from meta).
    - **`crates/cstat-core/src/inference.rs`**: extend `Predictor` with `total_session: Mutex<Session>`, load `total_model.onnx` in `Predictor::load`, add `predicted_total: f32` to `Prediction`, run the totals session in `predict()`. **Constraint**: totals model uses 58 features (49 diffs + 9 sums), not 49 — needs a separate `TOTAL_NUM_FEATURES` const and a parallel feature builder, or extension of the existing 49-feature builder to also produce the 9 sums (cleaner, since `features.rs::build_game_features` already has the home_/away_ team rows in scope). `predict_with_contributions` is margin-only — totals attribution has no story value (the keys panel is a margin narrative).
    - **`crates/cstat-api/src/routes/predict.rs`**: extend response with `predicted_total`, `predicted_home_score`, `predicted_away_score` (rounded to int). The neutralization helper at line ~314 currently does antisymmetric averaging for margin (`0.5 * (fwd - rev)`) — totals need *symmetric* averaging (`0.5 * (fwd + rev)`) since `total(A,B) == total(B,A)` by construction. Update the antisymmetry test (`predict.rs:704`) with a totals-symmetry analogue. The `predict_margin_and_winprob` helper (consumed by `routes/teams.rs::team_detail` for the Projected column and `routes/ticker.rs` for upcoming tiles) needs a totals-aware variant returning `(margin, win_prob, home_score, away_score)`.
    - **Frontend**: three surfaces, all already share `result.predicted_margin` / `result.home_win_probability`. (1) `web/src/pages/Predict.tsx:1402` headline chip → `Duke 75 — UNC 72` with margin/win% on a secondary line. (2) `web/src/components/ScoreTicker.tsx` upcoming tiles → projected score line under team names. (3) `web/src/pages/TeamDetail.tsx` Projected column → `75-72` instead of `−3.2`. Add a `formatProjectedScore(home, away, homeName, awayName)` helper colocated with `homeAdvantageSign` so all three surfaces format identically.
  - **Honest precision framing for the UI**: total mean 146.6, σ=18.9; backtest MAE 13.58 is materially worse than KenPom (~9) and Vegas (~7-8), so projected scores should be framed as KenPom-style approximations, not betting-grade precision. The MAE floor on diff+sum features at 3 seasons of data appears to be ~13.5 — the 9 sum features were added incrementally and the last 3 (`sum_w_ortg` + the two rebound rates) gained only ~0.02 MAE over the first 6, with `sum_w_ortg` not even cracking top-15 importance (redundant with `sum_adj_offense`). The next material lever is **full historical data** (§6, NatStat archive 2007→2026) — a separate multi-PR effort. Don't pour more time into feature engineering before that lands.
