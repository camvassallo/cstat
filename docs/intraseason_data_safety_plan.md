...
| **C2** | **Column merge** — `sync_to_prod.sh --columns table.col` | Manual, laptop | Named columns only, on any non-EXCLUDED table with a UNIQUE constraint (`players.display_name` is the motivating case) | `UPDATE … FROM (VALUES …)` in batches, matched on the natural key — **no TRUNCATE, no INSERT, no DELETE**, so it cannot cascade (`sync_to_prod.sh:430-563`) | **Safe in-season by construction** — strictly narrower than **C**, and the only route to a derived column on an FK-referenced table, which **C** would cascade-wipe (R3) and **B** is refused for. |
