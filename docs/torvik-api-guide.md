...
- **`player_id` still cannot be assumed unique per `(player, season)` on a database that has not had the #332 repair run.** The prune removes the stale cause and the linker fix in #313 removes the double-claim cause, but neither reaches a third: a link made before #243's two-candidate rule existed, which the current linker would not produce and, because the upsert keeps a prior link when a run fails to match, nothing would ever remove. `cstat-ingest torvik --year YYYY --reconcile-links report` names those rows by asking the linker and comparing; `--reconcile-links apply` clears them, and refuses when the set is more than 2% of the season's linked rows — a set that large means the roster the linker saw is what changed, not the links. Run it per season, report before apply, and on prod as well as locally, since prod re-fetches only the current season. Once every season on a database is clean the partial unique index `(player_id, season) WHERE player_id IS NOT NULL` becomes addable, and that is the thing that would make any recurrence loud. It is not added yet: a `UNIQUE` migration that fails on a prod with duplicates stops the API booting, so prod has to be repaired first.
