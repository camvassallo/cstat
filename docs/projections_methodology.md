...
w         = 0.30 for continuity rosters, ramping to 0.20 for roster overhauls
```

- **0.30 baseline weight, 0.0 offset (continuity teams)** — retuned 2026-08-23 on the end-to-end `cstat-ingest projections-backtest` (targets 2019–2026, n=2626), scored with the leave-one-season-out models so the figure is leak-free. roster-impact's raw output is a genuine projector (raw pooled MAE 5.90, bias +0.17 on the 11-season run), not a same-season descriptive model used out of context, so the blend leans far less on baseline persistence than the box-score pipeline did (0.30 vs 0.80) and needs **no calibration offset** — the box-score pipeline's `+2.0` corrected a structural −4.8 bias the box-score model had as a projector; the roster-impact model doesn't have it. **Honest LOSO pooled MAE 5.675** at the served ramp, beating a flat-0.5 blend (5.737), the LOSO-refit transition-conditional weight (5.679) and baseline-persistence (6.57). *(The superseded 0.45 came from the continuity cohort's optimum under the hindsight retention key — see the 2026-08-23 callout above for why that cohort was the wrong one to tune against. Prior regimes, for reference: 0.50 when the calibrator trained on the single-prior-season trajectory OOF and the backtest scored 2025+2026, n=496.)*
- **Turnover-conditional weight (overhaul teams)** — `baseline` (last season's AdjEM) is a *stale anchor* when a roster turns over wholesale, so the weight `w` ramps from 0.30 down to **0.20** as the team's *retained talent fraction* falls from 0.40 to 0.20. The fraction is `Σ |base-season cam_v3| of returners / Σ |base-season cam_v3| of (returners + departures)` — **magnitude, not signed sums** (#322): cam_v3 is centred near 0, so signed sums went negative on rebuilt rosters and voided the ratio on the very teams the ramp exists for. Magnitude reads as "how much of last year's on-court impact is coming back", which is the right question — a heavily-used *bad* player leaving makes last season's AdjEM exactly as stale as a star leaving does. Keyed on roster turnover alone, **not** `is_new_hc` — turnover is the stronger signal (it directly measures baseline staleness, and subsumes 61% of new-HC teams), has no false positives from same-roster coaching changes, and lives on `ProjectedRoster` so the route and `compute-projections` stay in lockstep with no DB fetch. **Worth +0.003 MAE over a retuned flat weight** on the 2019–2026 LOSO backtest — small, and much smaller than the +0.05 claimed before the retention key was fixed; it is retained for the near-zero overhaul-cohort bias it buys (−0.05 vs −0.15 flat) rather than for pooled accuracy. Constants + the ramp live in `roster_projection::{PROJECTION_SHRINK_WEIGHT_OVERHAUL, transition_shrink_weight, retained_talent_fraction}`; the per-team weight is surfaced on `ProjectedTeam.baseline_weight`.
