...
- The canonical drops the query string, any trailing slash, and a trailing `/index.html`, so `/`, `/index.html`, `/predict`, `/predict/` and `/predict?season=2025` collapse onto the indexable URLs the sitemap lists. `/index.html` gets an explicit route because `ServeDir` would otherwise serve that file raw — no canonical, and the file's static homepage `og:url` — leaving a second indexable URL for the homepage on each host.
- The `og:url` the server replaces is removed **by tag, not by line and not by quoting style**. A line filter looks equivalent while `index.html` is pretty-printed and fails silently the day anything minifies the head: the one line contains `og:url`, the whole region is dropped, and every non-entity page quietly stops emitting a canonical. `SpaTemplate::load` also warns loudly if the markers are present but the region does not parse.
- **Known trade-off:** an unrecognised path still returns 200 with `index.html` (unchanged — it is what lets React Router render its own not-found state), and now carries a self-referential canonical. Google soft-404s those on content rather than indexing them, and the alternative — enumerating known routes server-side — is a second copy of the router that would drift out of sync. Canonicalising everything is the safer default: failing to canonicalise a *real* page re-splits the hosts, which is the expensive direction.
