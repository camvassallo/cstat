...
- **Preflight command (1.2)** — `cstat-ingest preflight [--year] [--strict]` (`crates/cstat-ingest/src/preflight.rs`) probes all four feeds — Postgres (`SELECT 1`), NatStat (cheap cached `teamcodes` fetch = reachable + auth-valid), Torvik (lightweight `coachdict.json` status probe), and 247 (page-1 auth peek, only if a JWT is configured) — and prints a per-feed reachable/skipped/down table. Exits non-zero when a **serving-critical** feed (DB/NatStat/Torvik) is down, or with `--strict` when *any* feed (incl. 247) is down. The nightly orchestrator runs it as step 0: it logs the per-feed summary and records a `preflight` ledger row, but does **not** gate control flow — the existing per-step isolation already fail-softs best-effort feeds and hard-fails the serving-critical chain; a down critical feed just lands in the degraded Slack summary.
...
- **Torvik CSV schema guard (1.5)** — `validate_player_csv_schema` runs once on the first data row of the headerless positional CSV, asserting the *type shape* of load-bearing columns (name/team/conf are non-numeric text; gp/usage/pid/gbpm parse as numbers when present, empties tolerated). If Bart inserts/reorders a column a value lands in the wrong slot and the parse **fails loudly** instead of writing misaligned garbage. Covered by `torvik::tests::parse_csv_rejects_shifted_schema`.
