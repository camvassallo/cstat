...
- **Invariant gates (shared with M5 4.1)** — new `cstat_core::invariants::check_season` module: teams with box scores but NULL AdjEM, completed two-team games missing a `team_game_stats` side, W-L drift vs `team_game_stats.win`, and the two swapped-game detectors (issues #119/#140, kept in sync with `tests/swapped_games.rs`). The simulate CLI exits non-zero on any violation, hard-failed window, or idempotency drift — greppable/CI-able by construction. M5's post-run nightly gates will call the same module.
