...
- **Row-count sanity (4.1, second half)** — step 10 snapshots the season-scoped row count of every served table (`games`, `team_game_stats`, `player_game_stats`, `team_season_stats`, `player_season_stats`) into `ingest_run_table_counts` (migration `042`, excluded from `sync_to_prod.sh`) and compares against the most recent prior run. A **material** shrink — >5% **and** >25 rows (`detect_count_regressions`, both floors so early-season noise and a few repair-deletions don't false-fire) — degrades the run with a `row_counts: prev → cur` line. **A regressed snapshot is not persisted** (`snapshot_counts` reads, `persist_counts` writes, and the caller only writes on a clean gate): persisting it would make the corruption the baseline, so the next run would compare corrupt-to-corrupt, see no drop, and post a green heartbeat over a still-broken table — the gate would alert exactly once and then actively assert health. Holding the last-known-good baseline keeps it firing nightly until the counts recover. In-season these tables only ever grow, so a drop means a truncated feed or a compute that wiped rows. The first in-season run has no prior and just records the baseline. Pure comparison + `heal_window` logic unit-tested in `run_ledger.rs`; the gate + snapshot verified end-to-end through the `simulate` harness (4 runs × 5 tables persisted, all `row_counts` steps `ok`).
- **Backfill-gap self-heal (4.2)** — when the window is the **default** (the production cron path — the CLI passes `self_heal` only when `--from` is absent), the nightly first reads `last_successful_ingest_date` from the ledger (max `ended_at` of an `ok` `games` step — deliberately **not** season-scoped, since "when did the cron last succeed" is a property of the scheduler, not a season; scoping it would make the self-heal structurally dead across the Nov 1 rollover, where the new season has no ledger history and an outage spanning it would lose the season openers) and, if the cron missed one or more nights, widens the window start back to that date via `heal_window` (capped at 14 days so a long off-season silence can't trigger a months-wide NatStat pull). The re-covered dates are a harmless idempotent overlap; a fully-healed run stays a SUCCESS and notes the widening in its Slack summary. An operator's explicit `--from` is never auto-widened, and `simulate` passes `self_heal = false` (it drives its own clock). Verified end-to-end: backdating the sim ledger's last `games` success to 5 days ago made a default-window run widen `2026-07-16 → 2026-07-12` and proceed on the healed window. **Partial heals degrade the run:** when the 14-day cap bites (an outage longer than the cap), the dates between the last success and the capped start are *not* re-ingested and nothing would ever pick them up — so `heal_window` returns a `HealPlan { from, unrecovered_days }` and a non-zero `unrecovered_days` pushes a `self-heal only PARTIAL` line into `failures`, naming the exact `nightly --from … --to …` backfill to run. A silently-permanent hole in the served box scores is the one outcome this gate exists to prevent, so it must never read as a green "gap healed".
