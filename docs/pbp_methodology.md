...
- **Exact (`onfloor`)** — when a game's `play_by_play` rows carry the stored API `onfloorhome`/`onfloorvis` (nightly-forward + eventual backfill), the stint is built from them by **union reconstruction** (see below) — NOT a literal per-play read, because the source field is incomplete on ~half of plays.
