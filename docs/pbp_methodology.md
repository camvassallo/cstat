...
- Per team-game the loader picks `natstat_lineups` if gated, else `onfloor` if any row has it, else `replay` — a side has exactly one **served** source. Covered sides additionally keep their replay/onfloor reconstruction under the **`replay_shadow`** label (2026-06-12): excluded from every served rollup (`_game_lineups`, `lineup_aggregates`, `plus_minus_pbp`, `player_on_off` — no double-count), read only by the offline RAPM fit (`training/rapm.py`), which needs the opponent-paired stints the per-game natstat units don't carry (`docs/rapm_methodology.md` §3.2). `lineup_aggregates.source` records the best served source seen (so the UI can flag approximate data). The corrupt-PBP gate (2019) blocks only the replay path; gated natstat team-games still emit.
...
- **`scripts/sync_to_prod.sh` must be patched before the first PBP load.** Its auto-discovery loop would otherwise try to push the multi-GB raw table to Railway. Extend `EXCLUDED` (currently `api_cache`, `_sqlx_migrations`) with `play_by_play` and `lineup_stints`, comment each with the why, and print the excluded set in `--dry-run`. *(Done — `EXCLUDED` now also derives each excluded table's owned sequences; see issue #186 for why the table name alone was not enough.)*

> **CORRECTION 2026-07-17 — "the DB cap" was never quantified, and it does not exist.** This section originally justified the local-only split by claiming a push would "hit the DB cap". Priced against a real Railway invoice ($16.06/mo), storage costs **$0.15/GB/month**: the raw `play_by_play` table is **$1.44/month**, and full fidelity (`play_by_play` + `lineup_stints` + `natstat_lineups`) is **+$3.78/mo → ~$19.84 total**. Memory is 93% of that bill and disk is 4%; the API never queries PBP, so putting it on prod does not move the dominant line item. **One number here is also stale:** `play_by_play` measures **9,846 MB / 32.8M rows** (not ~7.5 GB). For scale, the whole syncable set is **2,637 MB** locally and prod's billed volume is **~2.54 GB** — those two agreeing within 1.4% is what confirms the invoice decode is reading the right units. The storage split below is therefore a *choice*, not a constraint — and ROADMAP Phase 6 *"Prod self-sufficiency"* now proposes reversing it, because the local-only rule is what forces a laptop to be online for `lineup_aggregates` / `player_on_off` to stay fresh. Treat the rule below as the current state, not as a justified design.
