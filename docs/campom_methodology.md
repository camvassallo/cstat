...
Consensus All-American teams (first and second) and AP Player of the Year,
2015–2026 — **125 award-seasons, all 125 resolved**. The data lives at
`data/awards/consensus_all_americans.csv`; load and score it with
`training/awards.py` (`load_awards()` / `score_metric()`), which owns the
name normalization, school disambiguation, and the issue-#243 pid overrides.

Scoring is NDCG@25 with tier weights (POY = 3, consensus 1st = 2, consensus
2nd = 1), plus plain recall of All-Americans into the top 10 / top 25. The
qualified pool is GP ≥ 5 and `minutes_per_game` ≥ 10 — note that this column
actually stores *share of team minutes*, not MPG (a known ingest bug, see
issue #243), so the cut is more permissive than it reads. It is applied
identically to every metric compared, so the comparison is unaffected.

### Headline result — CamPom clearly beats raw GBPM

| metric | raw `gbpm` | `cam_gbpm_v3_psos` | relative |
| --- | --- | --- | --- |
| NDCG@25 | 0.596 | **0.780** | **+31%** |
| All-Americans in top 10 | 0.380 | **0.538** | **+42%** |
| All-Americans in top 25 | 0.627 | **0.810** | +29% |
...
and so falls out of the pool — issue #243.)
