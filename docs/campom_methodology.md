...
normalization, school disambiguation, and the issue-#243 pid overrides.
...
that this column actually stores *share of team minutes*, not MPG (a known
ingest bug, see issue #243), so the cut is more permissive than it reads. It
is applied identically to every metric compared, so the comparison is
unaffected.
...
| --- | --- | --- | --- |
| NDCG@25 | 0.583 | **0.774** | **+33%** |
| All-Americans in top 10 | 0.284 | **0.441** | **+55%** |
| All-Americans in top 25 | 0.510 | **0.741** | +45% |
| POY ranked #1 | 0.545 | **0.636** | +17% |
| POY in top 5 | 0.727 | **0.909** | +25% |

Reproduce with `cd training && ./.venv/bin/python awards.py`. (POY rates are
over 11 of 12 seasons: Obi Toppin's 2020 row has a NULL `cam_gbpm_v3_psos`
and so falls out of the pool — issue #243. Recall figures are lower than under
a two-tier ground truth simply because 193 awardees compete for the same top-10
and top-25 slots; NDCG@25 is the comparable measure, and the CamPom-vs-GBPM
gap is stable at +0.18 to +0.19 across both tier configurations.)
