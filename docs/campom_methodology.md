...
## Validation — how well does CamPom identify the best players?

*(Study run 2026-07-31 over all 12 ingested seasons, 2015–2026.)*

CamPom is a **descriptive grade**: its job is that the best player in the
country carries the highest score, not that a roster's scores reconcile to
team results. Everything below is measured against that goal.

### Ground truth

All-America selections and AP Player of the Year, 2015–2026 — **193 rows over
12 seasons, all 193 resolved to a `torvik_pid`**. That is the 125 official
consensus first/second-team selections plus a 68-player **derived** third
team. The data lives at `data/awards/consensus_all_americans.csv`, is
regenerated by `training/build_awards_data.py`, and is loaded and scored with
`training/awards.py` (`load_awards()` / `score_metric()`), which owns the name
normalization, school disambiguation, and the `PID_OVERRIDES` table. Those
overrides no longer exist because rows are *unreachable* — #243 fixed the
linkage — but because the award selectors use a player's common name where
NatStat stores the legal one (Ja/Temetrius Morant, Obi/Obadiah Toppin). The
join could now go through `torvik_player_stats.player_name`, which carries
Torvik's common-name spelling; see the note on the table itself.

The NCAA recognizes only two consensus teams. The third tier extends the same
official point system (3/2/1 over AP, USBWA, NABC, Sporting News) one band
further. It is trustworthy because the builder's own gates require that the
*same* computation reproduce the officially published first and second teams
in all 12 seasons — which it does — before the third band is written. See
`data/awards/README.md`.

Scoring is NDCG@25 with tier weights (POY = 4, consensus 1st = 3, consensus
2nd = 2, derived 3rd = 1), plus plain recall of All-Americans into the top
10 / top 25. The qualified pool is GP ≥ 5 and `minutes_per_game` ≥ 10 — note
that this column actually stores *share of team minutes*, not MPG. That is a
column-naming misnomer rather than a live ingest bug (`total_minutes` holds
the true MPG and `compute_campom` reads the right column for each; the rename
is tracked as a #243 follow-up in ROADMAP), so the cut is more permissive than
it reads. It is applied identically to every metric compared, so the
comparison is unaffected.

### Headline result — CamPom clearly beats raw GBPM

| metric | raw `gbpm` | `cam_gbpm_v3_psos` | relative |
| --- | --- | --- | --- |
| NDCG@25 | 0.603 | **0.764** | **+27%** |
| All-Americans in top 10 | 0.274 | **0.442** | **+61%** |
| All-Americans in top 25 | 0.499 | **0.728** | +46% |
| POY ranked #1 | 0.583 | 0.583 | — |
| POY in top 5 | 0.750 | **0.917** | +22% |

Reproduce with `cd training && ./.venv/bin/python awards.py`. Recall figures
are lower than under a two-tier ground truth simply because 193 awardees
compete for the same top-10 and top-25 slots; NDCG@25 is the comparable
measure.

**Restated over 12 of 12 seasons (2026-08-01, issue #243).** These figures
were first published over **11** seasons: Obi Toppin's 2020 row had a NULL
`cam_gbpm_v3_psos` from the Torvik linkage bug and fell out of the qualified
pool entirely, taking the whole 2020 POY comparison with it. With that season
restored, **the POY@1 advantage disappears — raw GBPM ranks Toppin #1 in 2020
and CamPom ranks him 5th, so both metrics now sit at 7/12.** Everything else
holds or improves: NDCG@25 +0.16, top-10 recall +0.17, POY@5 +0.17. The
headline claim is unchanged and the ranking evidence is if anything cleaner,
but the specific "+17% on POY ranked #1" line from the original writeup was an
artifact of the excluded season and should not be cited.

The volume adjustment is what produces the gap. Raw GBPM crowns the
efficient low-usage role player — Gary Clark (20.8 usage) in 2018, Xavier
Tillman (21.6) in 2020. CamPom replaces them with Trae Young and Markus
Howard, and recovers Cooper Flagg (2025) and Cameron Boozer (2026) as
outright #1.

### Secondary strengths

| property | raw `gbpm` | CamPom | note |
| --- | --- | --- | --- |
| Year-over-year rank stability | 0.685 | **0.743** | returning players |
| NBA draft separation (AUC) | 0.908 | **0.921** | drafted vs undrafted |
| Transferable team value (R²) | 0.693 | **0.753** | prior-season roster → team AdjEM |

### Known weakness — do not use CamPom for team attribution

Aggregated over a roster to explain the **current** season's team quality,
CamPom is materially **worse** than raw GBPM (AdjEM R² 0.960 vs 0.977;
win% R² 0.454 vs 0.645). This is by design, not a defect: the volume
multipliers deliberately break the per-100-possession additivity that makes
raw GBPM aggregate cleanly to team margin. CamPom trades descriptive
accuracy for portability. **For "why was this team good," use raw GBPM;
for "how good is this player," use CamPom.**

### Tuning — the constants are validated; leave them alone

An 864-config grid over all seven constants, selected leave-one-season-out,
**scores worse than the shipped values** (0.762 vs 0.777). Naive in-sample
tuning would have reported 0.797 and looked like a +0.02 win — it is
selection noise. LOSO pick stability confirms it: five different configs
win across twelve folds.

Single-knob paired tests (one constant moved, others at production, signed
over 12 seasons) — nothing clears "positive, ≥ 8/12 wins, p < 0.10":

| change | ΔNDCG | W-L | p |
| --- | --- | --- | --- |
| `CAMPOM_SOS_TRANSFER_RATE` 0.5 → 0.75 | +0.0082 | 8-3 | 0.175 |
| `CAMPOM_DEFENSE_DISCOUNT` 0.1 → 0.2 | +0.0080 | 7-4 | 0.147 |
| `CAMPOM_PLAYER_SOS_TRANSFER_RATE` 0.15 → 0.30 | +0.0074 | 6-6 | 0.569 |
| **`CAMPOM_MINUTES_EXPONENT` 0.5 → 0.35** | **−0.0107** | **3-9** | **0.077** |
| `CAMPOM_SOS_TRANSFER_RATE` 0.5 → 0.25 | −0.0061 | 2-9 | 0.147 |
| `CAMPOM_PLAYER_SOS_TRANSFER_RATE` 0.15 → 0.05 | −0.0080 | 4-8 | 0.301 |

The most consistent finding in the study is that **weakening** any
adjustment hurts. Every hand-picked constant is defended from below and
none can be improved from above. Two leads sit at p ≈ 0.15 (conference SOS
up to 0.75, defense discount up to 0.2) — re-test as seasons accumulate,
but neither justifies a change today.

### Rejected extensions

Each was tested and did not survive:

- **Un-freezing `USG_REF`** to a per-season mean: −0.005, wins 1/12. The
  frozen reference is empirically better despite looking inconsistent.
- **`stops`** as an added term: flat-to-worse at every weight (0.796 →
  0.791–0.795), despite helping on draft and team-attribution targets.
- **Box / shot profile, height, `class_year`**: help only on NBA-draft
  targets. `class_year` is the single largest apparent "gain" (+0.041 AUC)
  and is pure draft-eligibility bias — freshmen are drafted at 5.20% vs
  seniors at 1.42% at equal production. Height contributes exactly zero to
  team attribution. Both are NBA-scouting artifacts, not college value.
- **RAPM / on-off inputs**: +0.002. Consistent with the RAPM verdict in
  `docs/rapm_methodology.md`.
- **A rate/value restructure** (splitting the signed rate from volume so
  roster values sum to team AdjEM): failed its acceptance gate and produced
  a 9/10 identical transfer leaderboard.
- **A free-form model over CamPom's own inputs**: 0.928 vs the formula's
  0.921 on draft AUC, and *worse* on value discovery — the hand-written
  formula is already at the ceiling of the information its inputs carry.

**Do not tune against NBA draft outcome or team attribution.** They pull in
opposite directions and neither is what CamPom is for: draft rewards being
young and tall, team attribution rewards the winningest role player. The
same constant grid produces opposite recommendations depending on which is
used. Awards are the anchor.
