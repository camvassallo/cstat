...
**No authorship or tooling attribution anywhere** — not in `pr.md`, not in PR bodies, not in commit messages. No "Generated with Claude Code", no robot emoji, no `Co-Authored-By` trailer, no "written by" line of any kind. The PR body describes the change and the commit message describes the commit; nothing else.
...
# rather than running trainers by hand — the chain is guard -> frame -> roster_impact ->
# roster_adjo -> roster_adjd -> backtest -> cae -> compute-projections, and hand-running it is what let
# `roster_adjo` serve a three-generation-stale OOF for months (#218). The three
# roster-frame models (net, AdjO, AdjD — #378) also stamp their meta with an OOF
# fingerprint and the API REFUSES TO BOOT if they disagree, so retraining one
# without the others is a hard failure, not a silent one.
...
# Materialize completed-game projections into `game_projections` (#266). The
# nightly does the CURRENT season on its own; run this by hand to backfill
# historical seasons (nothing else writes them, so a team page for 2019 projects
# its whole schedule live until you do) or to re-materialize after a retrain.
# ~20-40s per season — the cost is per CUTOFF DATE, not per game.
cargo run --bin cstat-ingest -- game-projections --years 2024,2025
...
- **cstat-ingest** — NatStat API client (`client.rs`), response cache (`cache.rs`), token-bucket rate limiter (`rate_limiter.rs`), and ingestion pipeline (`ingest/`). CLI binary at `src/bin/ingest.rs` with subcommands: `season`, `teams`, `players`, `team`, `games`, `perfs`, `update`, `elo`, `forecasts`, `compute`, `status`, `preflight` (connectivity health check — pings DB/NatStat/Torvik/247, exits non-zero on a serving-critical feed down), `clean-cache`, `torvik`, `campom-parity`, `explore`, `bootstrap-csv` (historical CSV bootstrap), `playbyplay` (intra-season PBP loader), `simulate` (M4 offline season-replay harness — replays a CSV-bootstrapped season through the real `nightly` orchestrator against the isolated `postgres-sim` DB via synthesized `api_cache` fixtures, with invariant checks per window + an idempotency re-run; the simulated clock advances through `cstat_ingest::today_utc()`, which also honors `CSTAT_SIMULATED_DATE`), `lineups` (NatStat `games;lineups`-object capture into the durable local-only `natstat_lineups` tables — restart-safe via its ledger, v4-pinned), `transfers` / `recruits` / `coaches` (247-portal / 247-recruit / coachdict ingest), `departures` / `departures-audit` (hand-curated non-portal, non-draft exits into `player_departures` from `data/departures/{year}_departures.json`, plus the offseason worklist that ranks the returners the projection still assumes are coming back — see `docs/projections_methodology.md`), `returns` (the inverse capture — players the `class_year == 'Sr'` inference deletes who are actually coming back under the NCAA 5-in-5 rule, loaded from `data/returns/{year}_returns.json` into `player_returns`; `granted` projects as an ordinary returner, `contested` goes to the uncertain `?` bucket and widens the team's floor/ceiling band — see `docs/eligibility_5in5.md`. A row is keyed by the player's BASE-season team but its status follows him through the portal: a `contested` mover is a `?` at his destination rather than a firm arrival, and an `Entered`-with-no-destination portal row is beaten by a curated row naming his old team (he is staying). **Both this loader and `departures` REPLACE the year rather than upserting into it**, so a row deleted from the JSON is deleted from the table — the file is the source of record in both directions. `returns --resolve-reason <reason> [--case "<suit>"] --as granted|departed` applies a court outcome to a cohort at once and stops without loading so the change lands as a reviewable diff; `--case` scopes it to one suit's plaintiffs, because the 2026 cohort rides on a federal class action plus two dozen state suits that resolve one court at a time. Named plaintiffs are sourced with `scripts/eligibility_litigation_worklist.py`), `rosters` (official school-published rosters into `team_roster_fetches` / `team_roster_players` — the only forward-looking roster signal cstat has, covering the four populations no ingested feed sees: redshirts staying put, D2/D3 up-transfers, JuCo arrivals, and direct international signings. **`--year` is the season the roster is FOR** (2027 = the 2026-27 rosters), the opposite convention to `departures`/`returns`. It feeds `departures-audit`, **not** the projection's scored roster — see `docs/official_rosters.md`), and the projection/eval tooling `projections-backtest`, `compute-projections`, `game-projections`, `measure-blend-accuracy`. **`season` is the bootstrap command** — it runs the seven NatStat steps, then Torvik, then `compute_all`, in one call. `update` likewise runs compute at the end by default. Both accept `--no-torvik` / `--no-compute` opt-outs. **`nightly` is the in-season production refresh** (`SeasonIngester::nightly`) — the served-critical subset (games/perfs/teamperfs by date range → forecasts → `/elo` ratings → Torvik with per-game persistence) **before** `compute_all`, recording each step to the `ingest_runs` ledger; window defaults to yesterday..today (UTC). Unlike `update` it refreshes Torvik + `/elo` first, so `cam_gbpm_v3`/`pit_cam_v3` and `team_season_stats.elo_rating` (the served `diff_elo_rating` feature) don't go stale on a recompute. It also refreshes the 247 **transfer portal** (two class years, incremental) and the 247 **recruit feeds** (rankings + commits, two class years, full sweep — 247 publishes no cursor) best-effort, using a **guest** JWT minted per run from a public 247 page — no `TFS_247_JWT` needed. The recruit classes are shifted one forward from the portal's (`self.season` / `self.season + 1`): class C first plays in season C+1, so those are the arriving class and the one being recruited. That reverses ROADMAP S5/P3, whose premises (no in-season portal churn; 247 can't be autonomous) both stopped holding under the 5-in-5 rule: 51 players entered the 2026 class in the first 19 days of August 2026. Recruits joined on the same reasoning once the JSON migration removed their credential. Both sit after everything served-critical and record their own `ingest_runs` steps (`transfers_{year}`, `recruits_{year}`, `recruit_commits_{year}`, `recruit_resolve`), none served-critical — a 247 outage is a failed ledger row, never a degraded box-score refresh. `recruit_resolve` is the one that is *not* merely best-effort: it makes no 247 call and is the only writer of `recruits.committed_team_id`, which the roster projection selects on, so a failure there silently drops recruits from projected rosters and is recorded and degraded rather than warned. After `compute_all` it also runs **`game_projections`** — the completed-game projection sweep that keeps `GET /api/teams/{id}` off the model (#266). Best-effort: a failure sends team pages back to the slow live path, not to a wrong one. It sweeps the **whole current season**, not just the night's games, because only the CamPom channel of the pit feature vector is genuinely point-in-time — team stats, roster aggregates and rolling form come from the season-aggregate tables `compute_all` rewrites, so every past game's projection moves nightly too. That is affordable because the sweep batches by **cutoff date** (one point-in-time cohort per date, ~150 a season, not one per game). Full plan: `docs/in_season_ingest_plan.md`; 247 endpoint map, auth tiers and payload quirks: `docs/247_api.md`. **`--year` defaults to `current_natstat_season()`** (date-derived in `crates/cstat-ingest/src/lib.rs`), so the binary stays correct as the calendar rolls. Single team-id resolver lives at `cstat_ingest::team_id_by_code_and_season`; don't inline the `(natstat_id, season)` lookup. The `Team` subcommand delegates to `SeasonIngester::ingest_team(code)` — keep new per-team orchestration there, not in the bin.
- **cstat-api** — Axum HTTP server. `AppState` holds `Database` + `NatStatClient` + `Predictor`. Routes under `/api/`. Health/observability routes (`/api/health`, `/api/status`, `/api/health/ingest`) are mounted **un-guarded** in `main.rs` so a saturated server or stale pipeline still answers — they bypass the cache/timeout/load-shed layer that wraps the data routes. `GET /api/health/ingest` (`routes/health.rs`) reports per-step freshness from `ingest_runs` and returns **503 when any served-critical step is >36h stale**, so an external uptime monitor catches a missed nightly. One exception, and it is the interesting one: a step whose latest ledger status is `source_not_published` — the nightly ran it correctly and the UPSTREAM source has no data for that season yet — gets its threshold widened by 14 days (`SOURCE_NOT_PUBLISHED_GRACE_HOURS`). That exists for the November flip, when `current_natstat_season()` rolls forward days-to-weeks before barttorvik publishes, and `torvik`/`torvik_games` (both served-critical) cannot succeed no matter what we do (#248). It widens the threshold rather than resetting the clock — `last_ok_at` still reads honestly — it is time-boxed so a source that never returns still goes red, and any other outcome reverts the step to the 36h rule immediately, so a real Torvik outage inside the window is not masked.
...
Postgres with SQLx. Migrations in `/migrations/` (53 files). Key tables: `teams`, `players`, `games`, `player_game_stats` (110+ columns), `player_season_stats`, `team_season_stats`, `team_game_stats`, `player_percentiles`, `game_forecasts`, `torvik_player_stats`, `torvik_player_game_stats` (point-in-time CamPom source), `player_archetypes`, `archetype_models`, `api_cache`, `transfers`, `recruits`, `draft_entrants`, `player_departures` (hand-curated non-portal/non-draft exits — pro signings abroad, retirements, dismissals; the projection's fourth departure channel, see `docs/projections_methodology.md`), `player_returns` (hand-curated eligibility returns — the 5-in-5 stay-put channel, see `docs/eligibility_5in5.md`), `trajectory_oof_predictions` / `freshman_oof_predictions` (held-out projection inputs), `team_preseason_projection` (materialized served AdjEM band), `game_projections` (materialized pre-game projection per completed game — the nightly writes it and `team_detail` reads it instead of running the model per game per page view, #266), `coaches` / `coach_seasons` (coachdict entity model), `team_roster_fetches` / `team_roster_players` (official school-published rosters — see `docs/official_rosters.md`; the **fetch status is load-bearing**, because a player's *presence* is a fact from one row while their *absence* is a claim about the whole page, and schools publish partial "(Returners)" pages and last season's roster all summer — only `status = 'ok'` licenses reading a departure from an absence), `coach_season_cae` / `coach_ratings` (coach-above-expectation grades), `ingest_runs` (per-step nightly-ingest ledger — runtime-written on prod, excluded from `sync_to_prod.sh`), `artifact_provenance` (which model artifact produced each Layer 3 derived product — see `docs/model_dependency_graph.md` §6).

All season-scoped tables carry a `season` column; the API and frontend support arbitrary multi-year browsing via a site-wide `?season=` query param plumbed through `web/src/components/season.ts::useSeason()`. **The frontend reads the season list from `GET /api/seasons`** (DISTINCT season FROM games, newest first) — no source edit needed for the dropdown when adding a year. Adding a new season is two commands: `cargo run --bin cstat-ingest -- season --year YYYY` (NatStat + Torvik + compute, end-to-end) and a `cd training && python -m archetypes --seasons …` retraining pass on the new combined cohort. Optional: ingest the 247Sports transfer portal for that class year via `cargo run --bin cstat-ingest -- transfers --year YYYY` (live API, needs `TFS_247_JWT`) or `--bootstrap-from data/transfers/YYYY_raw.json` to load a captured snapshot. Rows land in the `transfers` table; the `/api/transfers/{year}` route reads from there. **The recruit feeds need no credential** — `recruits --year YYYY` and `recruits --year YYYY --commits-feed` both read 247's JSON API on a guest token minted per run, so they can run unattended. `--html` selects the legacy scrape, which is the only source of `previous_rank` / `committed_school_slug` and needs a subscriber `TFS_247_COOKIE` for the composite half; the JSON upserts COALESCE those columns so a JSON pass never blanks what a scrape captured. **JSON `compositeRating` is 0–100 and the column is 0–1** — the parser rescales; scale trap and full field map in `docs/247_api.md`. **The neighbouring `compositeStarRating` is not the composite star and must not be read** — the two JSON feeds disagree with each other on it and both over-award 5-stars; `star_rating` is banded from the composite rating instead (`tfs_recruits::composite_star_rating`, floors 0.9900 / 0.9350 / 0.8100). Those floors are **fitted to the HTML-scraped history, not to 247's published scale** — only the 5-star floor matches a published figure, and the plausible-looking 0.8900 / 0.7900 for the lower two mis-band 17% of the history one star too high. Audit a class, and re-fit the floors after any 247 rescale, with `psql "$DATABASE_URL" -f scripts/audit_recruit_stars.sql` (query 3).
...
**Conference for a season that isn't ingested yet** lives in `data/conference_realignment.json` — a hand-curated diff of the moves for one target season, keyed by `teams.short_name`. The Future page projects a season before any `teams` row for it exists, so the DB's only answer is last year's league, which is wrong for the 31 programs that realigned into 2026-27 (30 conference changes plus one Division I departure). `fetch_conferences` (projections route) prefers the target season's own ingested + Torvik-corrected conference and falls back to base-season + this diff, so the capture retires itself the day the season lands. **One exception, and it is load-bearing**: `teams.conference` is written verbatim from NatStat at ingest, and NatStat mislabels realignment (the whole reason the Torvik correction exists). Torvik publishes weeks after `current_natstat_season()` rolls forward, so in that window the target-season row reports the league the team just *left* — taking it at face value would silently revert the column to the pre-move league on the default Future view for exactly the fortnight the move is newsworthy. So when the ingested value still equals the capture's `from`, the capture wins; any other value beats it, so this cannot pin a stale answer. **Every entry records the conference it leaves (`from`), and applies only while the base season still agrees** — a stale entry no-ops instead of asserting a fiction. `include_str!`-compiled via `cstat_core::realignment`, so editing it needs a **redeploy, not a sync** (same trap as `data/player_display_names.json`). Invariants — including that every destination is a code `TORVIK_CONF_TO_CSTAT` can also produce, which is what catches a league *rebrand* like WAC → UAC — are in `crates/cstat-core/tests/conference_realignment.rs`. Display and search only; no model reads a conference.
...
- `features.rs` — builds the 49-feature diff vector from DB (team stats, roster aggregates, rolling form); `build_all_features_pit` swaps the leaky season-aggregate CamPom channel for a point-in-time map when `as_of_date` is set. **That map is flattened into the SQL `UNNEST` in sorted `player_id` order, and the sort is load-bearing** — `HashMap` iteration order varies per instance, which changed the order Postgres summed the minutes-weighted aggregates in, moved their last bits, and flipped LightGBM splits: the same request returned 16.8 or 17.1 points depending on the run before it was fixed (#266). Note the pit path makes only the CamPom channel point-in-time; the other 41 features are season aggregates, which is a train/serve skew tracked in #274.
- `GET /api/predict?home=Duke+Blue+Devils&away=North+Carolina+Tar+Heels&neutral=false` — returns predicted margin + win probability; `&as_of_date=YYYY-MM-DD` serves point-in-time + preseason×pit-blended predictions, and `prediction_basis` (`preseason`|`blended`|`pit`|`leaky`|`cross_era`) labels the active regime. **`&home_season=` / `&away_season=` draw the two sides from different years** (the what-if matchup, ROADMAP §7c); each falls back to `season`, so a request naming neither is the legacy single-season path byte-for-byte. Differing seasons force a whole-season snapshot and are guarded accordingly: `as_of_date` alongside them is a **400** (the pit cohort spans one season and cannot straddle two), `is_conference_game` is forced to 0 (the 2015 Big East is not the 2026 Big East), prior meetings are skipped rather than queried, and the preseason blend is skipped — that last one is load-bearing rather than cosmetic, because when one slot is the in-progress season `apply_preseason_blend` would look up BOTH team ids in that single season. The early-season preseason blend (peak 0.70 at the Nov 1 open, linear decay to 0 over 42 days) also engages on the **live** path — no `as_of_date` computes the weight from `today_utc()`, so opening-week live predictions (Predict page, TeamDetail Projected column, ScoreTicker via `predict_projection`) anchor on the preseason projection instead of a 1–2 game sample; past the decay window it's a no-op.
...
- Projection models, in dependency order: `train_trajectory_model.py` (returner year-over-year CamPom, mean + q10/q90) and `train_freshman_model.py` (recruit first-season CamPom) are **Layer 1** — they persist held-out predictions to the `*_oof_predictions` tables; `train_roster_impact_model.py` (roster aggregate → team AdjEM, the served preseason calibrator — ordinary least squares on three CAM aggregates since #363, exported on the 27-slot ONNX contract; the LightGBM it replaced had a leaf ceiling) and `train_roster_adjo_model.py` / `train_roster_adjd_model.py` (the display-only AdjO and AdjD halves, same frame, reconciled to the net at serve time — #378) are **Layer 2**, trained on those OOF tables rather than on actuals — through a frame the Rust backtest cuts from the SERVED ex-ante roster composition (`projections-backtest --frame-out --frame-only`, `training/frames/`, gitignored); the trainers refuse a frame cut from a different OOF snapshot than the live tables. Every frame is checked by the look-ahead guard (`training/lookahead_guard.py`, #362: rebuild with the target season hidden behind a schema of views, features must be byte-identical; the `guard` stage of the chain). All leave-one-season/class-out backtested, and since #361 every trainer also reports **walk-forward** (train strictly earlier than the test season) through the shared `training/walk_forward.py` harness and stamps it into its meta as `walk_forward` — that is the headline number; `cd training && ./.venv/bin/python walk_forward_report.py` prints the tree-wide scorecard, and `retrain_downstream.sh` ends on it. Because Layer 2 trains on Layer 1's *predictions*, it absorbs upstream bias rather than compounding it — which makes the failure mode **desynchronization**, not bad data. Retrain from the highest stale node downward via `retrain_downstream.sh`; graph and protocol in `docs/model_dependency_graph.md`.
